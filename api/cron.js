...
export default async function handler(req, res) {
    try {
        ...
        const completion = await openai.chat.completions.create({
            model: "gpt-4o",
            messages: [{ role: "system", content: "Jsi The Hardware Guru. Piš v HTML." }, 
        ...
        const htmlPage = `<!DOCTYPE html><html lang="cs"><body><h1>${latestVideo.title}</h1>${content}</body></html>`;
        const ghUrl = `https://api.github.com/repos/zachranymail123-a11y/thehardwareguru-web/contents/public/clanky/${slug}.html`;

        const ghResponse = await fetch(ghUrl, {
            method: 'PUT',
            headers: { 'Authorization': `token ${githubToken}`, 'Content-Type': 'application/json', 'User-Agent': 'Vercel-Cron' },
            body: JSON.stringify({ message: `🤖 Automat: ${slug}`, content: Buffer.from(htmlPage).toString('base64') })
        });
