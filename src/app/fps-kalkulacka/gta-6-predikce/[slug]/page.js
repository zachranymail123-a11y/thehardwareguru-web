...
import ShareButtonsClient from './ShareButtonsClient'; // Vytvoříme klientskou komponentu pro interakci

/**
 * GURU GTA 6 PREDICTOR - V7.0 (VIRAL EDITION)
 * 🛡️ FIX: Přidán panel pro sdílení (Copy, X, Reddit).
...
export async function generateMetadata({ params, searchParams }) {
    return {
        title: `GTA VI FPS Prediction | Can I Run It? | Hardware Guru`,
        description: `Predicted performance for GTA VI on your hardware. Check out your estimated FPS!`,
    };
}
...
export default async function Gta6PredictionPage({ params, searchParams }) {
  ...
  if (!cpuId || !gpuId) return <div style={{color:'#fff', textAlign:'center', paddingTop:'100px'}}>Hardware IDs missing.</div>;
  ...
  const shareText = `🔮 GTA VI PREDIKCE: Moje sestava (${hwComboName}) by měla dát v GTA VI na ${resolution} okolo ${predictedFps} FPS! 🚀`;
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      <main style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 20px' }}>
        ...
        {/* 🔥 TADY JSOU TY TLAČÍTKA PRO SDÍLENÍ - VIRAL PANEL */}
