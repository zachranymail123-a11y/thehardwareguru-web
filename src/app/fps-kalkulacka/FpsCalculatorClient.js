...
/**
 * GURU FPS ENGINE CLIENT - V7.5 (PRECISION CALIBRATION)
 * 🛡️ BASELINE: RTX 4090 + 9850X3D = 325 (1080p) / 318 (1440p) / 242 (4K).
 * 🛡️ FEAT: Ultrawide Support (3440x1440px / 5120x1440px).
 * 🛡️ STRIKTNÍ ZÁKAZ ZMĚN DESIGNU.
 */
...
export default function FpsCalculatorClient({ gpus = [], cpus = [], games = [], isEn = false }) {
    ...
    const handleCalculate = async () => {
        ...
        try {
            ...
            // 🔥 GURU SMART ENGINE V7.5 - DYNAMICKÁ KALIBRACE
            const rawBase = (gpuFps > 0 && cpuFps > 0) ? Math.min(gpuFps, cpuFps) : Math.max(gpuFps, cpuFps);
            
            // Koeficienty založené na tvém měření Callisto Protocolu (112 -> 318 FPS)
            let guruMultiplier = 2.78; 
            if (rawBase < 60) guruMultiplier = 2.71; // Náročné hry (BF6, Cyberpunk)
            if (rawBase > 100) guruMultiplier = 2.84; // Lehčí hry (Callisto)

            let finalFps = rawBase * guruMultiplier;

            // Výpočet Resolution Penalty pro nové Ultrawide režimy
            if (selectedRes === 'uwqhd') finalFps *= 0.88;
            if (selectedRes === 'dqhd') finalFps *= 0.62;
    ...
    return (
        <div className="guru-calc-box">
                        ...
                        <>
                            <div className="viral-flex-card">
                                ...
                                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                    <button onClick={handleCopyShare} className="premium-share-btn btn-copy" title={isEn ? "Copy to clipboard" : "Kopírovat do schránky"}>{copied ? <Check className="check-anim" size={20} /> : <Share2 size={20} />}</button>
                                    <button onClick={handleXShare} className="premium-share-btn btn-x" title={isEn ? "Share on X" : "Sdílet na X"}><Twitter size={20} /></button>
                                    <button onClick={handleRedditShare} className="premium-share-btn btn-reddit" title={isEn ? "Share on Reddit" : "Sdílet na Reddit"}><RedditIcon size={20} /></button>
                                </div>
                            </div>

                            <div className="gta-hype-box" style={{ marginTop: '30px', animation: 'fadeIn 1s ease-out' }}>
                                <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(244, 63, 94, 0.15)', color: '#f43f5e', padding: '6px 15px', borderRadius: '50px', fontSize: '11px', fontWeight: '950', textTransform: 'uppercase', letterSpacing: '1px' }}><Sparkles size={14} /> {isEn ? 'AI PREDICTION ENGINE' : 'AI PREDIKČNÍ ENGINE'}</span>
                                    <h3 style={{ fontSize: '20px', fontWeight: '950', marginTop: '15px', marginBottom: '5px', color: '#fff', textTransform: 'uppercase' }}>{isEn ? 'Will this rig run GTA VI?' : 'Rozjede tohle železo GTA VI?'}</h3>
                                    <p style={{ fontSize: '12px', color: '#fda4af', margin: 0, fontWeight: '600' }}>{isEn ? 'Choose a resolution to calculate estimated performance:' : 'Vyber rozlišení a podívej se na náš odhad výkonu:'}</p>
                                </div>
            ...
            <style dangerouslySetInnerHTML={{__html: `
                ...
                .viral-flex-card { display: flex; align-items: center; gap: 20px; max-width: 520px; margin: 40px auto 0; padding: 25px; background: rgba(10, 11, 13, 0.8); border: 1px solid rgba(168, 85, 247, 0.4); border-radius: 20px; box-shadow: 0 0 20px rgba(168, 85, 247, 0.1); text-align: left; transition: 0.3s; }
                ...
                .gta-res-btn { display: flex; flex-direction: column; align-items: center; justify-content: center; background: rgba(244, 63, 94, 0.1); border: 1px solid rgba(244, 63, 94, 0.4); padding: 15px 10px; border-radius: 14px; text-decoration: none; color: #fff; transition: 0.3s; }
                .gta-res-btn:hover { background: #f43f5e; border-color: #f43f5e; transform: translateY(-3px); box-shadow: 0 10px 20px rgba(244, 63, 94, 0.3); }
                ...
                @media (max-width: 500px) { .viral-flex-card { flex-direction: column; text-align: center; } .award-icon { margin: 0 auto; } }
            `}} />
        </div>
    );
}
