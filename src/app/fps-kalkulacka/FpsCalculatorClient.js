...
/**
 * GURU FPS ENGINE CLIENT - V3.5 (PREMIUM VIRAL + X + REDDIT)
 * 🛡️ REDDIT SHARE: Přidáno nativní sdílení na platformu Reddit.
 * 🛡️ IKONA: Vložena bezpečná inline SVG ikona pro Reddit.
 */
...
// Bezpečná SVG ikona Redditu (pro jistotu, že nespadne build)
...
export default function FpsCalculatorClient({ gpus = [], cpus = [], games = [], isEn = false }) {
    ...
    // 1. Kopírování do schránky (pro Discord, FB)
    ...
    // 2. Nativní sdílení na X (Twitter)
    const handleXShare = () => {
        if (!result) return;
        const { gameName, cpuName, gpuName, url } = getShareDetails();
        ...
        <div className="guru-calc-box">
            <div className="guru-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
                ...
                <div className="input-field">
                    ...
                    <select value={selectedCpuId} onChange={(e) => setSelectedCpuId(e.target.value)} className="guru-select">
                        ...
                        {/* GURU: Data načítána striktně přes name */}
            ...
            <style dangerouslySetInnerHTML={{__html: `
                ...
                @media (max-width: 500px) {
                    ...
                    .premium-share-btn { width: 100%; max-width: 200px; height: 45px; }
                }
            `}} />
        </div>
    );
