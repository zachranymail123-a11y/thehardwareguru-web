...
/**
 * GURU FPS ENGINE - V1.2 (DATABASE CONNECTED)
 * 🛡️ FIX: Tahá reálné hry z tabulky 'games' a HW z 'cpus'/'gpus'.
 */
...
const fetchDatabase = async () => {
    ...
    try {
        const [gpuRes, cpuRes, gameRes] = await Promise.all([
            fetch(`${supabaseUrl}/rest/v1/gpus?select=id,name,vendor,slug,game_fps!gpu_id(*)&performance_index=gt.0&order=performance_index.desc`, { headers, cache: 'no-store' }),
            fetch(`${supabaseUrl}/rest/v1/cpus?select=id,name,vendor,slug,cpu_game_fps!cpu_id(*)&performance_index=gt.0&order=performance_index.desc`, { headers, cache: 'no-store' }),
