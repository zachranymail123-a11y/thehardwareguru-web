...
/**
 * GURU FPS ENGINE - V1.4 (ULTIMATE REPAIR)
 * 🛡️ FIX: Odstraněny Join relace z fetche, které způsobovaly prázdné seznamy.
 * 🛡️ ARCH: Čistý fetch HW dat zajistí, že se selecty vždy naplní.
 */
...
const fetchDatabase = async () => {
    ...
    try {
        // GURU FIX: Taháme čistá data bez joinů. Pokud join selže, Supabase vrátí prázdné pole. 
        // Tímto zajistíme, že se hardware v selectu VŽDY ukáže.
        const [gpuRes, cpuRes, gameRes] = await Promise.all([
            fetch(`${supabaseUrl}/rest/v1/gpus?select=id,name,vendor,slug,performance_index&performance_index=gt.0&order=performance_index.desc`, { headers, cache: 'no-store' }),
            fetch(`${supabaseUrl}/rest/v1/cpus?select=id,name,vendor,slug,performance_index&performance_index=gt.0&order=performance_index.desc`, { headers, cache: 'no-store' }),
...
export default async function FpsKalkulackaPage(props) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
        ...
        <div style={{ marginTop: '50px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            <a href={isEn ? "/en/bottleneck" : "/bottleneck"} className="silo-mini-card">
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <Info size={20} color="#a855f7" /> {isEn ? 'Check Bottleneck' : 'Zkontrolovat Bottleneck'}
                </div>
                <ArrowRight size={16} />
            </a>
            <a href={isEn ? "/en/cpuvs/ranking" : "/cpuvs/ranking"} className="silo-mini-card">
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <Cpu size={20} color="#f59e0b" /> {isEn ? 'CPU Ranking' : 'Žebříček procesorů'}
                </div>
                <ArrowRight size={16} />
            </a>
        </div>
      ...
      <style>{`
        .silo-mini-card { display: flex; align-items: center; justify-content: space-between; background: rgba(15,17,21,0.9); padding: 25px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05); text-decoration: none; color: #fff; font-weight: 900; text-transform: uppercase; font-size: 13px; transition: 0.3s; }
