...
/**
 * GURU CPU PERFORMANCE ENGINE V2.8 (ULTIMATE CONVERSION MACHINE)
 * 🚀 CÍL: Stabilní SEO CTA, Pulse Anchor Trap, Price Context a Scroll Trigger CTA.
 */
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: isEn 
      ? `${cpu.name} price, performance & benchmarks (2026) | The Hardware Guru`
      : `${cpu.name} cena, výkon a srovnání (2026) | The Hardware Guru`,
    alternates: {
...
export default async function CpuPerformancePage({ params }) {
  ...
  // 🔥 FIX #1: Deterministické CTA (SEO STABLE) 🔥
  ...
  return (
    <div className="guru-performance-wrapper" style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '160px', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      {/* 🔥 FIX #3: PULSING ANCHOR TRAP 🔥 */}
      {!isEn && (
        <div className="anchor-trap-mobile" style={{ position: 'fixed', bottom: '110px', right: '15px', zIndex: 9999 }}>
            <a href={heurekaLink} target="_blank" rel="nofollow sponsored" className="heureka-hn-link pulse-button" data-trixam-positionid="276026" data-trixam-content="Text link" data-trixam-medium="affiliate" style={{ background: '#0078d4', color: '#fff', padding: '12px 18px', borderRadius: '14px', fontSize: '13px', fontWeight: '900', textDecoration: 'none', boxShadow: '0 10px 40px rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid rgba(255,255,255,0.2)' }}>
        ...
        <section style={{ marginBottom: '40px' }}>
            <div className="benchmark-result-box" style={{ background: 'rgba(15, 17, 21, 0.95)', border: '1px solid rgba(245, 158, 11, 0.2)', borderLeft: '8px solid #f59e0b', borderRadius: '24px', padding: '50px 40px', boxShadow: '0 30px 70px rgba(0,0,0,0.7)', textAlign: 'center' }}>
                ...
                <div style={{ marginTop: '40px', textAlign: 'center' }}>
                  <a 
                    href={isEn ? amazonLink : heurekaLink} 
                    target="_blank" 
