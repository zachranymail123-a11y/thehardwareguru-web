...
import { ChevronLeft, Info, Calendar, Flame, Heart } from 'lucide-react';
import { createClient } from '@supabase/supabase-js';

/**
 * GURU ARTICLE ENGINE V3.2 (DYNAMIC RECIRCULATION & SELF-EXCLUSION)
 * Cesta: src/app/clanky/[slug]/page.js
 * 🚀 CÍL: Maximální prolinkování novinek a AdSense výdělky.
 * 🛡️ FIX 1: Striktní 'await props.params' pro Next.js 15.
 * 🛡️ FIX 2: Dynamické načítání 3 článků s filtrem .neq('id', post.id), aby nenabízel sám sebe.
 * 🛡️ FIX 3: Plná podpora CZ/EN varianty.
 */
...
// 🚀 GURU ENGINE: Funkce pro načtení 3 nejnovějších článků (kromě toho aktuálního)
const getLatestPosts = async (excludeId) => {
    const { data, error } = await supabase
        .from('posts')
        .select('title, title_en, slug, slug_en, created_at, image_url')
        .neq('id', excludeId) // 🛡️ GURU FIX: Tady filtrujeme aktuální článek!
...
export async function generateMetadata(props) {
    ...
    return {
        ...
        alternates: {
            ...
            languages: {
                'en': `${baseUrl}/en/clanky/${post.slug_en || post.slug}`,
                'cs': canonicalUrl,
...
export default async function ArticleDetailPage(props) {
    ...
    if (!post) {
        return <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><h1>404 | ČLÁNEK NENALEZEN</h1></div>;
    }

    // 🚀 Načteme ty 3 další články
    ...
    const date = new Intl.DateTimeFormat(isEn ? 'en-US' : 'cs-CZ', { year: 'numeric', month: 'long', day: 'numeric' }).format(new Date(post.created_at || Date.now()));
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
            <main style={{ maxWidth: '900px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
                ...
                {/* 🚀 DYNAMICKÁ SEKCE NEJNOVĚJŠÍCH ČLÁNKŮ (Nahrazuje ten statický balast z obrázku) */}
                                ...
                                return (
                                    <a key={lp.slug} href={lpUrl} className="related-article-card">
                                        <div className="related-img-wrapper">
                                            <img src={lp.image_url || fallbackImg} alt={lpTitle} />
                                        </div>
            ...
            <style dangerouslySetInnerHTML={{__html: `
                ...
                .guru-article-content h2 { color: #fff; font-size: 1.8rem; font-weight: 950; margin: 1.5em 0 0.8em; text-transform: uppercase; }
                .guru-article-content p { margin-bottom: 1.2em; }
                .guru-article-content a { color: #a855f7; text-decoration: none; font-weight: bold; }
