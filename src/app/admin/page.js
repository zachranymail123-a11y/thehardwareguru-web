...
export default function AdminApp() {
  ...
  const fetchIntelFeed = async () => {
    ...
    // HW a Game zdroje fungují přes rss2json v pořádku
    const HW_FEEDS = [
    ...
    try {
      // Běžný fetch pro propustné RSS (HW, Games)
      const fetchSet = async (list, type) => {
        const results = await Promise.all(list.map(async f => {
          try {
            const r = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(f.url)}&t=${Date.now()}`);
            const d = await r.json();
            return (d.items || []).map(item => ({ ...item, source: f.name, intelType: type }));
          } catch (e) {
            console.warn(`Guru Feed Warning (${f.name}):`, e.message);
      ...
      const fetchLeaksBackend = async () => {
        try {
          ...
          if (json.data && json.data.length > 0) {
             addLog("Úniky načteny z backendu.", "success");
             return json.data;
          }
      ...
      addLog(`Detekováno ${uniqueHw.length} HW, ${uniqueGame.length} her a ${uniqueLeaks.length} úniků.`, 'warning');
  ...
  const createDraftFromIntel = async (item) => {
    ...
    try {
      ...
      // 🚀 GURU: Správné přiřazení typu Leaks
