...
// --- GURU CONFIG & ENV ---
// 🚀 GURU: Pre OpenAI používame tvoj kľúč z prostredia
...
export default function AdminApp() {
  ...
  const fetchAndScanData = async () => {
    ...
    setLoading(true);
    addLog('Skenujem Guru systémy...', 'info');
    try {
      ...
      addLog('Databáza synchronizovaná.', 'success');
  ...
  const fetchIntelFeed = async () => {
    ...
    addLog('Sťahujem intel z Tom\'s Hardware...', 'warning');
    try {
      ...
      if (resData.status === 'ok') {
        ...
        addLog(`Načítaných ${resData.items.length} správ.`, 'success');
      } else throw new Error('RSS nedostupné');
    } catch (err) { addLog(`Chyba feedu: ${err.message}`, 'error'); }
    finally { setIntelLoading(false); }
  };

  // --- 🚀 GURU: OPENAI TRANSLATOR (GPT-4o) ---
  const createDraftFromIntel = async (item) => {
    const openAiKey = getEnv('NEXT_PUBLIC_OPENAI_API_KEY');
    if (!openAiKey) {
      addLog('Chýba NEXT_PUBLIC_OPENAI_API_KEY v env!', 'error');
      return;
    }

    setIsTranslating(true);
    addLog(`GPT-4o pripravuje český koncept...`, 'warning');
    ...
    try {
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-4o-mini", // Guru: gpt-4o-mini je rýchly a lacný, vymeň za gpt-4o pre max kvalitu
          messages: [
            {
              role: "system",
              content: "Jsi Hardware Guru. Píšeš pro českou komunitu hráčů a geeků. Tvůj styl je profesionální, úderný a expertní. Vždy vracíš POUZE čistý JSON."
            },
            {
              role: "user",
              content: `Přelož a uprav tento článek z Tom's Hardware do češtiny. 
              Vrať JSON s poli: 
              title_cs (chytlavý nadpis), 
              seo_description_cs (popis pro Google), 
              content_cs (článek v HTML formátu, použij h2 pro podnadpisy, odstavce, odrážky. Aspoň 400 slov.),
              slug_cs (url slug z nadpisu).
      ...
      setPreviewMode('card');
      addLog('Koncept pripravený v náhľade.', 'success');
    } catch (err) {
      addLog(`OpenAI fail: ${err.message}`, 'error');
    } finally {
  ...
  const publishDraft = async () => {
    if (!draft) return;
    addLog('Publikujem do DB...', 'warning');
    try {
  ...
  const markAsFired = async (id, table) => {
    ...
    addLog('Skryté.', 'success');
    fetchAndScanData();
  };

  const clearQueueItems = async (list, tabName) => {
    if (!confirm(`Vymazať ${tabName}?`)) return;
    ...
    addLog('Vyčistené.', 'success');
    fetchAndScanData();
  };

  const executeSocial = async (item, type) => {
    setActiveTab('terminal');
    addLog(`Odpálenie: ${item.title}`, 'warning');
    ...
    fetch(webhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...item, guru_type: type, fired_at: new Date().toISOString() }) })
      .then(() => { supabase.from(type === 'deal' ? 'game_deals' : 'posts').update({ is_fired: true }).eq('id', item.id); addLog('Hotovo!', 'success'); fetchAndScanData(); })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0b0d', display: 'flex', color: '#fff', fontFamily: 'sans-serif' }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .preview-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.95); z-index: 200; display: flex; flex-direction: column; padding: 40px; overflow-y: auto; backdrop-filter: blur(15px); }
        .preview-window { background: #0a0b0d; border-radius: 30px; border: 4px solid #333; margin: 0 auto; overflow: hidden; transition: 0.3s; width: 100%; max-width: 1200px; min-height: 800px; }
        ...
        <div className="preview-overlay">
          <div style={{ maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px' }}>
                <button onClick={() => setPreviewMode('none')} className="sidebar-btn" style={{ width: 'auto', background: '#333' }}><ArrowLeft size={16}/> Späť</button>
                <div style={{ display: 'flex', gap: '15px' }}>
                    <button onClick={() => setPreviewMode(previewMode === 'card' ? 'slug' : 'card')} className="sidebar-btn" style={{ width: 'auto', background: '#a855f7', color: '#fff' }}>
                        {previewMode === 'card' ? 'Detail Článku' : 'Karta na Home'}
                    </button>
                    <button onClick={publishDraft} className="sidebar-btn" style={{ width: 'auto', background: '#10b981', color: '#fff' }}><Check size={16}/> PUBLIKOVAŤ</button>
                </div>
            </div>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '20px' }}>
                <button onClick={() => setPreviewDevice('desktop')} style={{ padding: '10px', background: previewDevice === 'desktop' ? '#eab308' : '#222', borderRadius: '8px' }}><Monitor/></button>
                <button onClick={() => setPreviewDevice('mobile')} style={{ padding: '10px', background: previewDevice === 'mobile' ? '#eab308' : '#222', borderRadius: '8px' }}><Smartphone/></button>
            </div>
                   ...
                   <div style={{ padding: '40px', display: 'flex', justifyContent: 'center' }}>
                      <div className="mock-card">
                         ...
                         <div style={{ padding: '20px' }}>
                            <h3 style={{ color: '#fff', fontSize: '1.2rem', margin: '0 0 10px 0', fontWeight: '900' }}>{draft.title_cs}</h3>
                            <span style={{ color: '#66fcf1', fontWeight: 'bold', fontSize: '12px' }}>ČÍST VÍCE →</span>
                         </div>
                      </div>
                   </div>
                ) : (
                   <div style={{ padding: '60px 40px', maxWidth: '800px', margin: '0 auto' }}>
                      <h1 style={{ color: '#fff', fontSize: '3rem', fontWeight: '950', textTransform: 'uppercase', marginBottom: '30px' }}>{draft.title_cs}</h1>
      ...
      <aside className="admin-sidebar">
        ...
        <nav style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <SidebarItemUI id="pub-plan" activeTab={activeTab} setActiveTab={setActiveTab} icon={<Send />} label="Publikácia & Siete" color="#f97316" />
          ...
          <SidebarItemUI id="seo-hub" activeTab={activeTab} setActiveTab={setActiveTab} icon={<Globe />} label="SEO & Preklady" color="#eab308" />
          <SidebarItemUI id="automation" activeTab={activeTab} setActiveTab={setActiveTab} icon={<Zap />} label="Automatizácia" color="#a855f7" />
          ...
          <SidebarItemUI id="kal" activeTab={activeTab} setActiveTab={setActiveTab} icon={<CalendarDays />} label="Herný Kalendár" color="#3b82f6" href="/kalendar" />
        </nav>
      </aside>
      ...
      <main className="admin-main">
        ...
        {activeTab === 'dashboard' && (
          <div className="fade-in">
            <h2 className="tab-title" style={{ fontSize: '32px', fontWeight: 950, marginBottom: '30px' }}>STATUS</h2>
            <div className="stats-grid">
              <div className="stat-card"><h3>{data.stats.visits}</h3><p>NÁVŠTEVY</p></div>
              <div className="stat-card"><h3>{data.posts.length}</h3><p>ČLÁNKOV</p></div>
        ...
        {activeTab === 'intel-hub' && (
          <div className="fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px' }}>
              ...
              <button onClick={fetchIntelFeed} disabled={intelLoading} className="sidebar-btn active" style={{ width: 'auto', padding: '10px 20px', background: '#eab308', color: '#000' }}>
              ...
              {intelFeed.map((item, i) => (
                <div key={i} className="hub-card">
                  ...
                  <div style={{ marginTop: 'auto', display: 'flex', gap: '10px' }}>
                    ...
                    <button onClick={() => createDraftFromIntel(item)} disabled={isTranslating} className="action-btn-small" style={{ flex: 1.5, borderColor: '#eab308', color: '#eab308' }}>PŘIPRAVIT KONCEPT</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'pub-plan' && (
          <div className="fade-in">
            ...
            <div className="section-box">
              <h3 style={{ color: '#10b981', marginBottom: '20px' }}>Fronta na odeslání</h3>
        ...
        {activeTab === 'automation' && (
          <div className="stats-grid">
            <div className="stat-card"><h3>TIP GEN</h3><button onClick={() => runApiTask(`${BASE_URL}/api/generate-tip`, 'Tip Gen')} style={{ marginTop: '15px', padding: '10px', background: '#eab308', border: 'none', borderRadius: '8px' }}>SPUSTIŤ</button></div>
            <div className="stat-card"><h3>MAIN CRON</h3><button onClick={() => runApiTask(`${BASE_URL}/api/cron`, 'Main Cron')} style={{ marginTop: '15px', padding: '10px', background: '#ef4444', border: 'none', borderRadius: '8px' }}>SPUSTIŤ</button></div>
          </div>
        )}
      </main>
    </div>
