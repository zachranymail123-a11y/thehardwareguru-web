...
export async function GET(request) {
    ...
    try {
      ...
      const html = `
        ...
        <head>
            ...
            <title>Hardware Guru - IndexNow Automat (YEP)</title>
            <style>
                body { font-family: monospace; background: #0a0b0d; color: #66fcf1; padding: 20px; line-height: 1.5; }
                h1 { color: #f97316; text-transform: uppercase; border-bottom: 2px solid #333; padding-bottom: 10px; }
        ...
        <body>
            <h1>🚀 Guru IndexNow Automat (YEP)</h1>
            ...
            <p style="color: #f87171;">⚠️ NEZAVÍREJ TUTO STRÁNKU, automat právě sype data na Yep...</p>
            ...
            <script>
                ...
                async function processAll() {
                    ...
                    log('🔥🔥 HOTOVO! Celý web byl kompletně natlačen do Yepu! 🔥🔥', 'success');
                }
  ...
  try {
    ...
    if (!sitemapResponse.ok) return NextResponse.json({ success: false, message: "Podsitemapu se nepodařilo stáhnout." }, { status: 500 });
    ...
    if (urlList.length === 0) return NextResponse.json({ success: true, message: "Prázdná sitemapa, ignoruji.", totalUrlsProcessed: 0 });

    const chunkSize = 10000;
    const chunks = [];
    for (let i = 0; i < urlList.length; i += chunkSize) chunks.push(urlList.slice(i, i + chunkSize));

    const results = [];
    for (let i = 0; i < chunks.length; i++) {
        const payload = { host: host, key: key, keyLocation: keyLocation, urlList: chunks[i] };
        
        const response = await fetch('https://indexnow.yep.com', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json; charset=utf-8', 'Host': 'indexnow.yep.com' },
            body: JSON.stringify(payload)
        });

        results.push({ batch: i + 1, sentUrls: chunks[i].length, status: response.status });
        if (i < chunks.length - 1) await new Promise(resolve => setTimeout(resolve, 1000)); 
    }

    return NextResponse.json({ success: true, message: "Úspěšně odesláno.", totalUrlsProcessed: urlList.length, results: results });
  } catch (error) {
