export const maxDuration = 60; // GURU FIX: Zabrání timeoutu
    ...
    try {
      const [steamReq, tweakReq] = await Promise.all([
        fetch('https://google.serper.dev/search', {
          ...
          body: JSON.stringify({ q: `${rawTitle} minimum recommended system requirements pc Steam`, gl: 'us', hl: 'en', num: 4 })
        }),
        fetch('https://google.serper.dev/search', {
          method: 'POST',
          headers: { 'X-API-KEY': process.env.SERPER_API_KEY, 'Content-Type': 'application/json' },
          body: JSON.stringify({ q: `"${rawTitle}" PC performance "Engine.ini" OR "Registry" OR "Config" Reddit Steam Community`, gl: 'us', hl: 'en', num: 8 })
        })
      ]);
      ...
      openai.chat.completions.create({
        ...
        messages: [
          { 
            ...
            1. 'official_title': Oprav vstupní název hry. Použij správná velká a malá písmena (např. The Legend of Khiimori, Resident Evil Requiem, GTA V).
            2. 'Systémové požadavky': Vypiš reálné komponenty POUZE ze [STEAM DATA]. NIKDY si nevymýšlej procesory nebo grafiky.
            3. 'Hardcore Fixy': Kódy musí být KOMPLETNÍ. Vypiš přesnou cestu a minimálně 4 reálné technické parametry do Markdown bloku.
            4. 'Nastavení ve hře': Napiš 3-5 konkrétních položek s největším dopadem na VRAM a přesně jak je nastavit.
            5. 'EXPERT ZÓNA': Musíš vypsat PLNOU cestu v registrech a PLATNÝ klíč (DWORD) s hodnotou. Pokud v rešerši chybí registry fix, VŽDY vypiš reálný univerzální Windows gaming tweak.` 
          },
          { 
            role: "user", 
            content: `Hra: ${rawTitle}
...
{
  "official_title": "[ZDE NAPIŠ OFICIÁLNÍ NÁZEV HRY SE SPRÁVNOU GRAMATIKOU]",
  "meta_title": "Optimalizace ${rawTitle} - Expert Guru Guide",
  "seo_description": "Brutální optimalizace ${rawTitle}. Registry fixy, Engine.ini tweaky a hardcore nastavení pro maximální FPS.",
  "seo_keywords": "${rawTitle}, optimalizace, registry tweak, expert zona, hardware guru",
  ...
  "title_en": "${rawTitle} Hardcore Optimization Guide",
  "slug_en": "${slug}-optimization-guide",
  "meta_title_en": "${rawTitle} FPS Boost & Registry Surgery",
  "description_en": "No generic advice. Exact registry keys, full file paths and technical engine tweaks for ${rawTitle}.",
  "seo_keywords_en": "${rawTitle}, tweak, registry edit, pc guide",
  "content_en": "<h2>Guru Analysis</h2>\\n<p>[WRITE FULL ANALYSIS]</p>\\n<h2>System Requirements (Steam)</h2>\\n<ul>[WRITE HW REQUIREMENTS]</ul>\\n<h2>Hardcore Fixes and Optimization</h2>\\n<p>[WRITE FULL PATHS AND CODE BLOCKS WITHOUT SHORTENING]</p>\\n<h2>In-game Settings: What Kills FPS</h2>\\n<p>[SPECIFIC SETTINGS]</p>\\n<h2>EXPERT ZONE: Registry and File Modifications</h2>\\n<p>[WRITE FULL REGISTRY PATH AND EXACT KEYS]</p>\\n<p>Watch live on <a href='https://kick.com/TheHardwareGuru'>Kick</a>.</p>"
    ...
    return NextResponse.json({ ...ai, image_url: finalImg, title: ai.official_title || rawTitle });
