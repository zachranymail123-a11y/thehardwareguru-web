...
export async function GET() {
  try {
    ...
    // YouTube RSS (Doplň si své ID kanálu místo UC_ID)
    ...
    if (duplicate) return NextResponse.json({ status: 'Hotovo', video: video.title });
    ...
    await supabase.from('reports').insert([{ 
      title: video.title, video_id: video.id, content: completion.choices[0].message.content, url: video.link 
    }]);
