...
/**
 * GURU FPS ENGINE - GAME BENCHMARK PAGE V1.3 (CZ/EN MULTILANGUAGE)
 ...
 * 🛡️ FIX 1: Kompletní CZ/EN překlady podle zavedeného Guru standardu.
 * 🛡️ FIX 2: Ošetření 'en-' prefixu ve slugu, aby nerozbil PostgREST lookup.
 */
...
export async function generateMetadata({ params }) {
  const { gpu: rawGpuSlug, game: rawGameSlug } = params;
  ...
  return {
    title: isEn 
      ? `${gpu.name} ${gameLabel} FPS – 1440p Gaming Benchmark | The Hardware Guru`
      : `${gpu.name} ${gameLabel} FPS – Herní Benchmark 1440p | The Hardware Guru`,
    description: isEn
      ? `See ${gpu.name} real-life performance in ${gameLabel}. Average ${fps} FPS at 1440p resolution. Detailed gaming benchmark analysis.`
      : `Podívejte se na reálný výkon ${gpu.name} ve hře ${gameLabel}. Průměrně ${fps} FPS v rozlišení 1440p. Detailní analýza herního benchmarku.`
  };
}
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
        ...
        <section style={{ marginBottom: '60px' }}>
            <div className="content-box-style">
                <div className="guru-prose">
                    ...
                    {isEn ? (
                      <>
                        <p>According to our technical database and aggregated benchmarks, the <strong>{normalizeName(gpu.name)}</strong> delivers a solid gaming experience in <strong>{cleanGameLabel}</strong>. At 1440p resolution with High/Ultra settings, you can expect an average of <strong>{fps1440p} FPS</strong>.</p>
                      ...
                      <>
                        <p>Podle naší technické databáze a agregovaných benchmarků poskytuje <strong>{normalizeName(gpu.name)}</strong> vynikající herní zážitek ve hře <strong>{cleanGameLabel}</strong>. Při rozlišení 1440p a vysokých až ultra detailech můžete očekávat průměrně <strong>{fps1440p} FPS</strong>.</p>
        ...
        <section style={{ textAlign: 'center', marginTop: '80px' }}>
            ...
            <a href={isEn ? "/en/gpuvs" : "/gpuvs"} style={{ display: 'inline-flex', alignItems: 'center', gap: '12px', padding: '18px 40px', background: 'linear-gradient(135deg, #a855f7 0%, #7e22ce 100%)', color: '#fff', borderRadius: '16px', fontWeight: '950', fontSize: '15px', textDecoration: 'none', textTransform: 'uppercase', boxShadow: '0 10px 30px rgba(168, 85, 247, 0.3)' }}>
