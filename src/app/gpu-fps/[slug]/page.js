   ...
   <div className="guru-fps-wrapper" style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '160px', color: '#fff', fontFamily: 'sans-serif' }}>
     <main className="inner-container" style={{ maxWidth: '1200px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
       ...
       <header style={{ textAlign: 'center', marginBottom: '40px' }}>
         <div className="hunter-badge"><Gamepad2 size={16} /> GURU FPS HUNTER</div>
       ...
       <div className="affiliate-cta-grid" style={{ marginBottom: '50px', borderColor: `${vendorColor}40` }}>
           <div className="affiliate-col">
               <div className="affiliate-col-title" style={{ color: vendorColor }}><ShoppingCart size={16} /> {isEn ? `BUY ${normalizeName(gpu.name)}` : `KOUPIT ${normalizeName(gpu.name)}`}</div>
               <div className="affiliate-btn-wrap">
                   {isEn ? (
                       ...
                       <>
                           ...
                           <a href={`https://www.heureka.cz/?h%5Bfraze%5D=${encodeURIComponent(searchName)}+cena#utm_source=thehardwareguru.cz&utm_medium=affiliate&utm_campaign=25842&utm_content=v10-gpu-fps-top`} className="guru-buy-winner-btn heureka-btn v10-hl-btn" data-subid="v10-gpu-fps-top" data-cat="gpu_fps_hunter" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', textDecoration: 'none' }}><ShoppingCart size={16} /> Heureka.cz</a>
                       </>
                   )}
               </div>
           </div>
           ...
           return (
             <Link key={game.id} href={`/${isEn ? 'en/' : ''}gpu-fps/${safeSlug}/${game.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
               <div className="game-fps-card" style={{ background: 'rgba(15, 17, 21, 0.95)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px', padding: '30px', position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: '0.3s' }}>
                 ...
                 <div className="fps-val-main" style={{ fontSize: '64px', fontWeight: '950', color: '#fff', lineHeight: '1' }}>{fpsValue || '??'} <span style={{ fontSize: '20px', color: '#4b5563' }}>FPS</span></div>
                 <div style={{ marginTop: '15px', color: verdict.color, fontSize: '12px', fontWeight: '950', textTransform: 'uppercase', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span>{verdict.text}</span><ChevronRight size={16} /></div>
               </div>
             </Link>
           );
         })}
