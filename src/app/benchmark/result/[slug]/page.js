...
import { Trophy, Cpu, Zap, Twitter, Facebook, Layers, Gamepad2, Share2, ArrowRight, Activity, BookOpen, Swords, ShieldCheck, Lightbulb, Zap as Flash, GraduationCap, Link2 } from 'lucide-react';
...
export default async function ResultPage({ params }) {
    ...
    if (isCpuBottleneck) {
        ...
        evalDesc = `Tvůj procesor ${result.cpu_name} je pro grafiku ${result.gpu_name} jako zatažená ruční brzda. GPU musí neustále čekat na data, což ti drasticky sráží FPS i stabilitu (0.1% lows). Tohle nevyřešíš nastavením grafiky, tady potřebuješ víc jader a vyšší IPC!`;
        evalColor = "#ff0055";
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', marginTop: '25px' }}>
                        <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" style={{ background: '#1877f2', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', transition: '0.3s' }} className="hover:scale-110"><Facebook fill="#fff" stroke="none" /></a>
                        <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}`} target="_blank" style={{ background: '#000', border:'1px solid #333', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', transition: '0.3s' }} className="hover:scale-110"><Twitter fill="#fff" stroke="none" /></a>
                        <a href={`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}`} target="_blank" style={{ background: '#ff4500', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', transition: '0.3s' }} className="hover:scale-110"><RedditIcon size={26} /></a>
                    </div>
                </div>

                {/* 2. MASIVNÍ GURU HUB - PROLINKOVÁNÍ NAHORU */}
                <div style={{ marginBottom: '40px', background: 'rgba(15, 17, 21, 0.7)', border: '1px solid rgba(255,255,255,0.1)', padding: '30px', borderRadius: '24px', backdropFilter: 'blur(15px)', textAlign:'center' }}>
                    <h2 style={{ fontSize: '20px', fontWeight: '950', color: '#fff', marginBottom: '25px', textTransform: 'uppercase', letterSpacing: '1px' }}>PROZKOUMEJ CELÝ GURU EKOSYSTÉM</h2>
                ...
                {/* 4. DETAILNÍ ZHODNOCENÍ - ROZVEDENÉ */}
