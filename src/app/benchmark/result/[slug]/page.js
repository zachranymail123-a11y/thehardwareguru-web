...
import { Trophy, Share2, Activity, Cpu, Zap, ShoppingCart, Search, Crosshair, Swords, BookOpen, ArrowRight, Twitter, Facebook, ChevronRight, AlertTriangle, Gamepad2, Layers } from 'lucide-react';
...
export default async function ResultPageCZ({ params }) {
    ...
    let evalText = "Sestava je vyvážená!";
    let evalColor = "#10b981";
    if (isCpuBottleneck) { evalText = "⚠️ MASIVNÍ CPU BOTTLENECK!"; evalColor = "#ff0055"; }
    else if (isGpuBottleneck) { evalText = "⚠️ MASIVNÍ GPU BOTTLENECK!"; evalColor = "#f59e0b"; }
        ...
        <div style={{ background: '#0a0b0d', color: '#fff', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', backgroundPosition: 'center', minHeight: '100vh', padding: '120px 20px 60px', fontFamily: 'sans-serif' }}>
            ...
            <div style={{ maxWidth: '900px', margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                    <div style={{ color: '#a855f7', fontWeight: '950', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '10px' }}><Trophy size={18} style={{display:'inline', verticalAlign:'middle', marginRight:'5px'}}/> Oficiální GURU Výsledek</div>
                    <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: '950', textTransform: 'uppercase', margin: 0, textShadow: '0 0 20px rgba(168,85,247,0.4)' }}>
                        {result.nickname}
                    </h1>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '40px' }}>
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" style={{ background: '#1877f2', color: '#fff', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px' }}><Facebook fill="currentColor" stroke="none" /></a>
                    <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}`} target="_blank" style={{ background: '#000', color: '#fff', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', border: '1px solid #333' }}><Twitter fill="currentColor" stroke="none" /></a>
                    <a href={`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}`} target="_blank" style={{ background: '#ff4500', color: '#fff', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px' }}><RedditIcon size={24} /></a>
                </div>
                ...
                <div style={{ background: 'rgba(15, 17, 21, 0.9)', border: '1px solid rgba(168, 85, 247, 0.3)', borderRadius: '30px', padding: '50px 40px', textAlign: 'center', marginBottom: '40px', backdropFilter: 'blur(15px)', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}>
                    <div style={{ fontSize: '18px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '2px' }}>Celkové GURU Skóre</div>
                    <div style={{ fontSize: '7rem', fontWeight: '950', color: '#fff', margin: '10px 0', lineHeight: '1', textShadow: '0 0 40px rgba(168, 85, 247, 0.6)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginTop: '40px' }}>
                        <div style={{ background: 'rgba(255, 0, 85, 0.05)', border: '1px solid rgba(255, 0, 85, 0.2)', padding: '25px', borderRadius: '20px' }}>
                            ...
                            <div style={{ color: '#ff0055', fontSize: '28px', fontWeight: '950' }}>{result.cpu_score.toLocaleString()}</div>
                        ...
                        <div style={{ background: 'rgba(16, 185, 129, 0.05)', border: '1px solid rgba(16, 185, 129, 0.2)', padding: '25px', borderRadius: '20px' }}>
                            ...
                            <div style={{ color: '#10b981', fontSize: '28px', fontWeight: '950' }}>{result.gpu_score.toLocaleString()}</div>
                ...
                {/* EVALUACE A UPGRADY */}
                <div style={{ background: 'rgba(255,255,255,0.03)', border: `2px solid ${evalColor}`, borderRadius: '24px', padding: '40px', textAlign: 'center', marginBottom: '40px' }}>
                    <h2 style={{ fontSize: '26px', fontWeight: '950', color: evalColor, marginBottom: '15px' }}>{evalText}</h2>
                    <p style={{ color: '#9ca3af', fontSize: '1.1rem', marginBottom: '30px' }}>Na základě tvého testu doporučujeme tyto komponenty pro maximální FPS:</p>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '50px' }}>
                    <h2 style={{ fontSize: '24px', fontWeight: '950', color: '#fff', marginBottom: '30px', textAlign: 'center', textTransform: 'uppercase' }}>PROZKOUMEJ CELÝ GURU EKOSYSTÉM</h2>
