...
export async function generateMetadata({ params }) {
  ...
  const title = isEn && radaItem.title_en ? radaItem.title_en : (radaItem.title || 'Rada');
  ...
  return {
    ...
    openGraph: {
      title,
      description: desc,
      images: radaItem.image_url ? [radaItem.image_url] : [],
    }
  };
}
...
export default async function RadaDetail({ params }) {
  ...
  // Ochrana proti neexistující radě
  if (error || !radaItem) {
    notFound();
  }
  ...
  // 🚀 GURU FIX: Podpora jak 'content', tak 'description' sloupce
  const content = isEn 
      ...
      <main style={{ 
        ...
        {radaItem.image_url && (
          <div style={{ width: '100%', height: '450px', position: 'relative', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <img src={radaItem.image_url} alt={title || 'Náhled'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ...
        <div style={{ padding: '40px 50px 60px 50px' }}>
          ...
          {/* --- 🚀 GURU AFFILIATE NÁKUPNÍ BOX (Specifický pro danou radu) --- */}
          {radaItem.affiliate_link && (
            <div style={{ 
          ...
          <div style={{ 
            ...
            <h4 style={{ color: '#9ca3af', fontSize: '15px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px', margin: 0, textAlign: 'center' }}>
              {isEn ? "Did this guide help you? Support us either by buying games at the best prices or directly." : "Pomohla ti tato rada? Podpoř nás buď nákupem her za ty nejlepší ceny, nebo přímo."}
            </h4>
