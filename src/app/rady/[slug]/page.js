...
export default async function RadaDetail({ params }) {
  ...
  // 1. GURU FETCH: Získání dat z databáze rady
  ...
  const seoDesc = isEn && radaItem.seo_description_en ? radaItem.seo_description_en : (radaItem.seo_description || '');
  ...
  const productSchema = {
    ...
    "@type": "Product",
    "name": title,
    "image": [radaItem.image_url || `${baseUrl}/logo.png`],
    "description": seoDesc,
  ...
  const articleSchema = {
    ...
    "description": seoDesc,
    "image": [radaItem.image_url || `${baseUrl}/logo.png`],
    "datePublished": radaItem.created_at || new Date().toISOString(),
    "dateModified": radaItem.created_at || new Date().toISOString(),
    ...
    "mainEntityOfPage": { "@type": "WebPage", "@id": `${baseUrl}/${isEn ? 'en/' : ''}rady/${slug}` }
  };
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      {
        "@type": "Question",
        "name": isEn ? `How to apply this guide for ${title}?` : `Jak použít tento návod pro ${title}?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": isEn 
            ? `Follow the step-by-step instructions provided by the Hardware Guru experts for optimal results.` 
            : `Pro optimální výsledky postupujte podle kroků popsaných experty z Hardware Guru v tomto návodu.`
        }
      }
    ]
  };
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": title, "item": `${baseUrl}${isEn ? '/en' : ''}/rady/${slug}` }
    ]
  };
        ...
        <div style={{ padding: '40px 50px 60px 50px' }}>
          ...
          <div style={{ marginTop: '70px', paddingTop: '50px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '25px' }}>
            <h4 style={{ color: '#9ca3af', fontSize: '15px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px', margin: 0, textAlign: 'center' }}>
              {isEn ? "Did this guide help you? Support us by buying games at the best prices." : "Pomohla ti tato rada? Podpoř nás nákupem her za ty nejlepší ceny."}
            </h4>
