...
/**
 * GURU HTML SITEMAP V2.1 (STABILITY & SEO FIX)
 * 🛡️ FIX: Oprava přístupu k params (Next.js 15 asynchronní standard).
 * 🛡️ FIX: Oprava logiky jazykových verzí, aby nezpůsobovala Server Error.
 */
...
export async function generateMetadata() {
  // Mapa stránek je v základu CZ, EN verze je v /en/sitemap
  const title = 'Mapa stránek a Navigace | The Hardware Guru';
  const desc = 'Kompletní přehled všech rubrik, benchmarků, procesorů a grafických karet na The Hardware Guru.';
...
export default async function SitemapPage() {
  // Tato stránka je CZ verze. EN verze by měla být v src/app/en/sitemap/page.js
  const isEn = false; 
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Mapa stránek", "item": `${baseUrl}/sitemap` }
    ]
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{ marginTop: '80px', paddingTop: '50px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '25px' }}>
          <h4 style={{ color: '#9ca3af', fontSize: '15px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px', margin: 0, textAlign: 'center' }}>
            Podpoř projekt Hardware Guru a získej ty nejlepší slevy.
          </h4>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '20px', width: '100%' }}>
            ...
            <Link href="/support" className="guru-support-btn" style={{ flex: '1 1 280px' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @media (max-width: 768px) { .guru-deals-btn, .guru-support-btn { width: 100%; font-size: 14px; } }
      `}} />
    </div>
  );
}
