 ...
 * 🚀 CÍL: Oprava EN verze, Amazonu, Heureka V10 a Google Golden Rich.
 */
...
export async function generateMetadata(props) {
    ...
    const canonicalUrl = `https://thehardwareguru.cz${isEn ? '/en' : ''}/gpu-performance/${safeSlug}/${gameSlug}/${resolution}`;

    return {
        title: isEn ? `${gpu.name} ${gameLabel} FPS ${resLabel} Benchmark | Hardware Guru` : `${gpu.name} ${gameLabel} FPS (${resLabel} Test) | Hardware Guru`,
        alternates: {
            canonical: canonicalUrl,
            languages: { "en": `https://thehardwareguru.cz/en/gpu-performance/${safeSlug}/${gameSlug}/${resolution}`, "cs": `https://thehardwareguru.cz/gpu-performance/${safeSlug}/${gameSlug}/${resolution}` }
        }
    };
}
...
export default async function GpuPerformancePage(props) {
    ...
    const gameSlug = p?.game || '';
    const resolution = p?.resolution || '';
    // 🔥 FIX: Detekce jazyka z URL i z proxy
    const isEn = props.isEnProxy === true || rawSlug.startsWith('en-');
    const gpuSlug = rawSlug.replace(/^en-/, '');

    const data = await getPerformanceData(gpuSlug, gameSlug, resolution);
    if (!data) return <div style={{ color: '#ff0055', textAlign: 'center', padding: '100px', fontWeight: '950' }}>404 - DATA NENALEZENA</div>;
    ...
    const jsonLd = {
        ...
        "@type": "Article",
        "headline": `${gpu.name} ${gameLabel} FPS Benchmark in ${resLabel}`,
        ...
        "description": `Detailed gaming performance and FPS analysis of ${gpu.name} in ${gameLabel} at ${resLabel} resolution.`
    };
