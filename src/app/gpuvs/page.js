...
// Fallbacky pro bezpečný běh v Canvasu
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setError(isEn ? "Database connection unstable. Refresh page." : "Synchronizace s databází selhala. Zkuste obnovit stránku.");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#0a0b0d] text-white font-sans selection:bg-[#ff0055]" style={{ backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '140px', paddingBottom: '100px' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .guru-start-btn:disabled { opacity: 0.3; cursor: not-allowed; transform: none !important; }
        ...
        <section className="guru-glass-card mb-24">
            ...
            {gpuA === gpuB && gpuA !== '' && (
              <p className="text-[#ff0055] text-[10px] font-black text-center mt-6 uppercase tracking-[0.3em]">
                {isEn ? "CRITICAL ERROR: SELECT DIFFERENT HARDWARE!" : "KRITICKÁ CHYBA: VYBERTE RŮZNÝ HARDWARE!"}
              </p>
            )}
        </section>
            ...
            <div className="grid grid-cols-1 gap-4">
              {existingDuels.length > 0 ? existingDuels.map((duel) => (
                ...
                <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px] bg-black/20 text-gray-600 font-black uppercase tracking-[0.5em] text-xs italic">
                   {isEn ? "Database is empty. Start the first battle!" : "Databáze je prázdná. Odpal první souboj!"}
                </div>
              )}
            </div>
          )}
