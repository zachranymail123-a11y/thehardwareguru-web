...
/**
 * GURU GPU DUELS ENGINE - DETAIL V90.0 (ULTIMATE EDITION)
 * Cesta: src/app/gpuvs/[slug]/page.js
 * 🛡️ FIX 1: Žádná zrádná Next.js cache (revalidate = 0, no-store fetch).
 * 🛡️ FIX 2: Všudypřítomný safeSlug (odstranění chyb s /undefined URL u Deep Dive a Upgradů).
 * 🛡️ FIX 3: Čisté fallbacky v tabulce parametrů (skrytí jednotek při chybějící hodnotě).
 * 🛡️ FIX 4: Dynamické generování FPS odkazů v Deep Dive sekci podle reálných dat z DB.
 */
...
// 🚀 GURU: Agresivní slugify engine s GPU specifiky
const slugify = (text) => {
    return text
      .toLowerCase()
...
const getSafeGpuSlug = (gpu) => {
    if (!gpu) return '';
    if (gpu.slug) return gpu.slug;
    return slugify(gpu.name).replace(/^rtx/,'geforce-rtx').replace(/^radeon/,'amd-radeon');
};
...
async function generateAndPersistDuel(slug) {
  ...
  try {
    ...
    const payload = {
        ...
        seo_description_cs: `Srovnání ${cardA.name} vs ${cardB.name}. Detailní parametry a herní výkon.`, 
        seo_description_en: `Comparison of ${cardA.name} vs ${cardB.name}. Detailed specs and gaming performance.`,
...
export default async function GpuDuelDetail({ params }) {
  ...
  // 🚀 GURU FIX: Bezpečný Upgrade URL link
  const upgradeUrl = winner && loser 
    ? `/${isEn ? 'en/' : ''}gpu-upgrade/${getSafeGpuSlug(loser)}-to-${getSafeGpuSlug(winner)}`
    : null;
  ...
  const productSchemaA = gpuA ? { "@context": "https://schema.org", "@type": "Product", "name": gpuA.name, "brand": gpuA.vendor, "category": "Graphics Card" } : null;
  const productSchemaB = gpuB ? { "@context": "https://schema.org", "@type": "Product", "name": gpuB.name, "brand": gpuB.vendor, "category": "Graphics Card" } : null;
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
        ...
        {/* 🚀 GURU FIX: DEEP DIVE CROSS-LINKS SE SAFESLUG A DYNAMICKÝMI HRAMI */}
        <section style={{ marginBottom: '60px' }}>
          <h2 className="section-h2" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '25px' }}>
              {[gpuA, gpuB].filter(Boolean).map((gpu, i) => {
                  const safeSlug = getSafeGpuSlug(gpu);
                  const gpuFpsData = gpu?.game_fps && Array.isArray(gpu.game_fps) && gpu.game_fps.length ? gpu.game_fps[0] : (gpu?.game_fps || {});
                  const availableGames = Object.keys(gpuFpsData)
                  ...
                  const gamesList = availableGames.length > 0 ? availableGames.slice(0, 2) : ['cyberpunk-2077', 'warzone'];
                  ...
                  <div key={i} className="fps-matrix-card">
                      ...
                      <div className="matrix-links">
                          <a href={`/${isEn ? 'en/' : ''}gpu/${safeSlug}`} className="matrix-link">
                              <Monitor size={14} /> {isEn ? 'Full Profile' : 'Kompletní Profil'}
                          </a>
                          <a href={`/${isEn ? 'en/' : ''}gpu-performance/${safeSlug}`} className="matrix-link">
                              <BarChart3 size={14} /> {isEn ? 'Performance Specs' : 'Výkon a Parametry'}
                          </a>
                          <a href={`/${isEn ? 'en/' : ''}gpu-recommend/${safeSlug}`} className="matrix-link">
                              <ShieldCheck size={14} /> {isEn ? 'Guru Verdict' : 'Guru Verdikt'}
                          </a>
                          {gamesList.map((game) => (
                              <a key={game} href={`/${isEn ? 'en/' : ''}gpu-fps/${safeSlug}/${game}`} className="matrix-link">
                                  <Gamepad2 size={14} /> {game.replace(/-/g, ' ').toUpperCase()} FPS
                              </a>
                          ))}
                      </div>
                  </div>
