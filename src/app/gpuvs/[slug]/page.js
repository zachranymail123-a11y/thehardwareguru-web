...
/**
 * GURU GPU DUELS ENGINE - MASTER LOGIC V5.5 (ULTIMATE PRODUCTION)
 * Cesta: src/app/gpuvs/[slug]/page.js
 * Funkce: Automatická detekce jazyka (prefix en-), sjednocené SEO, Versus design.
 * Oprava: Absolutní prevence TypeError u SwG, fix Hydrace a DB Sync.
 */

// 🚀 GURU BUILD SHIELD: Bezpečné načtení modulů pro prostředí náhledu i produkce
...
export async function generateMetadata({ params }) {
  ...
  const { data: duel } = await supabase
    .from('gpu_duels')
    .select('title_cs, title_en, seo_description_cs, seo_description_en, slug')
    .eq('slug', slug)
    .single();

  if (!duel) return { title: 'Duel nenalezen | The Hardware Guru' };

  // Detekce jazyka: Pokud slug začíná 'en-', chceme anglická metadata
...
export default async function App({ params }) {
  ...
  if (!supabase) {
    return <div className="p-20 text-center text-white font-black uppercase">Chyba: Databáze není nakonfigurována v ENV.</div>;
  }

  // 1. GURU FETCH: Získání dat duelu z existujícího sloupce 'slug'
  ...
  // 2. DETEKCE JAZYKA (podle předpony slugu)
  const isEn = slug.startsWith('en-');
  ...
  return (
    <main className="min-h-screen text-neutral-200 py-12 px-4 sm:px-6 lg:px-8" style={{ 
      ...
      {/* 🛡️ GURU HYPER-SHIELD: Okamžitá ochrana proti pádu Google Subscribe scriptu */}
      ...
      <article className="max-w-5xl mx-auto">
        ...
        <header className="mb-12 text-center">
          <div className="flex items-center justify-center gap-4 text-neutral-400 text-sm font-black uppercase tracking-[0.3em] mb-6">
            ...
            {/* 🚀 GURU SHIELD: suppressHydrationWarning zabraňuje pádu do černé obrazovky */}
        ...
        <section className="mt-20 p-12 bg-neutral-900 border-2 border-orange-500/50 rounded-[50px] text-center shadow-[0_25px_60px_rgba(249,115,22,0.2)] relative overflow-hidden">
            ...
            <p className="text-neutral-400 mb-10 max-w-2xl mx-auto font-medium text-lg italic leading-relaxed">
              {isEn ? "We found the best deals for you. Instant key delivery and Guru-verified store." : "Našli jsme pro tebe ty nejlepší ceny na trhu. Okamžité doručení klíče a Guru-ověřený obchod."}
            </p>
        ...
        <footer className="mt-24 pt-12 border-t border-white/5 flex flex-col items-center gap-8">
            <h4 className="text-neutral-500 font-black uppercase tracking-[0.3em] text-xs text-center">{isEn ? "Help us build this database by supporting us." : "Líbí se ti tyto duely? Podpoř chod našich serverů."}</h4>
            <div className="flex flex-wrap justify-center gap-6 w-full">
              <a href="https://www.hrkgame.com/#a_aid=TheHardwareGuru" target="_blank" rel="nofollow sponsored" className="flex-1 min-w-[280px] py-6 px-10 bg-neutral-800 border border-white/10 rounded-2xl flex items-center justify-center gap-4 font-black uppercase tracking-widest text-orange-500 hover:bg-neutral-700 transition-all shadow-xl">
                <Flame size={22} /> {isEn ? 'BEST GAME DEALS' : 'HRY ZA NEJLEPŠÍ CENY'}
              </a>
              <Link href={isEn ? "/en/support" : "/support"} className="flex-1 min-w-[280px] py-6 px-10 bg-yellow-500 text-black rounded-2xl flex items-center justify-center gap-4 font-black uppercase tracking-widest hover:bg-yellow-400 transition-all shadow-xl">
