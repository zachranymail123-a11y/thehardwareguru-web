...
import SupportClient from '@/components/SupportClient'; // Předpokládáme existenci nebo vnitřní komponentu

/**
 * GURU SUPPORT ENGINE V2.0 (GOLDEN RICH RESULTS FIX)
 * Cesta: src/app/support/page.js
 * 🚀 CÍL: 100% zelená v Google Search Console.
 * 🛡️ FIX 1: Přechod na Server Component pro Metadata a JSON-LD (Wikipedia model).
 * 🛡️ FIX 2: Implementace Golden Rich standardu (Organization, FAQ, Breadcrumbs).
 * 🛡️ FIX 3: ESM importy místo require() dle ChatGPT instrukcí.
 */
...
export default async function SupportPage(props) {
  ...
  // 🚀 ZLATÁ GSC SEO SCHÉMATA (Organization & FAQ)
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* 🚀 GURU: Vložení klientské části (QR, SWG tlačítko, Stripe) */}
      <SupportPageContent isEn={isEn} />
...
// --- 🚀 VNITŘNÍ KLIENTSKÁ KOMPONENTA (PRO SWG A INTERAKCI) ---
// V produkci bys ji mohl dát do samostatného souboru, ale pro Canvas ji necháme zde.
function SupportPageContent({ isEn }) {
      ...
      <div style={{ marginBottom: '12px' }}>
        <div className="guru-btn-hover" style={{ ...buttonBaseStyle, backgroundColor: '#161920', position: 'relative', overflow: 'hidden' }}>
           ...
           <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, opacity: 0.001, zIndex: 10 }}>
              <button id="support-page-swg-btn" swg-standard-button="contribution"></button>
           </div>
        </div>
      </div>
