...
// Pomocná funkce pro vyčištění názvu pro Heureku
...
export async function generateMetadata({ params }) {
    ...
    // 🔥 ROBUSTNÍ DETEKCE EN JAZYKA
    const isEn = String(p.slug).startsWith('en-') || p.lang === 'en';
...
export default async function BottleneckPage({ params }) {
  ...
  // 🔥 FINÁLNÍ FIX DETEKCE JAZYKA (Next.js 14 programmatic safe)
  // Pokud params.slug obsahuje 'en-' nebo se nacházíme v /en/ složce
  const isEn = String(p.slug).startsWith('en-') || p.lang === 'en';
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
        ...
        <header style={{ textAlign: 'center', margin: '50px 0' }}>
          <div style={{ color: '#66fcf1', border: '1px solid rgba(102,252,241,0.3)', padding: '6px 20px', borderRadius: '50px', fontSize: '11px', fontWeight: 950, display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
            <Gauge size={16} /> 
            <span>GURU REVENUE ENGINE V4.0</span>
          </div>
        ...
        <div style={{ textAlign: 'center', fontSize: '14px', fontWeight: 900, marginBottom: '20px', color: '#facc15' }}>
          🔥 {isEn ? 'BEST UPGRADE RECOMMENDATION' : 'NEJLEPŠÍ UPGRADE PRO TUTO SESTAVU'}
        </div>
        ...
        <section className="affiliate-cta-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', padding: '35px', background: 'rgba(0,0,0,0.5)', borderRadius: '28px', border: '1px solid rgba(168,85,247,0.2)' }}>
            ...
            <div className="affiliate-col" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
                <div style={{ color: '#a855f7', fontWeight: 950, fontSize: '13px', textTransform: 'uppercase' }}>
                    <Monitor size={16} style={{ verticalAlign: 'middle', marginRight: '8px' }} />
                    {isEn ? 'GPU UPGRADE' : 'DOPORUČENÝ UPGRADE GRAFIKY'}
                </div>
                <div style={{ opacity: 0.6, fontSize: '12px' }}>{isEn ? 'From $499' : 'Guru cena od 11 990 Kč'}</div>
                <div style={{ fontSize: '11px', color: '#facc15', fontWeight: 900 }}>
                    {isEn ? `Losing ${bottleneckPercent}% FPS performance` : `Ztrácíš až ${bottleneckPercent}% výkonu`}
                </div>
                <div style={{ background: 'rgba(34,197,94,0.1)', borderRadius: '12px', padding: '12px', fontWeight: 900, width: '100%', textAlign: 'center', border: '1px solid rgba(34,197,94,0.2)', color: '#22c55e' }}>
                    🚀 {isEn ? `60 FPS → ${afterFps} FPS` : `60 FPS → ${afterFps} FPS po upgradu`}
                </div>
                <div style={{ fontWeight: 900, color: '#a855f7' }}>🔥 {targetGpuName}</div>
                <a href={isEn ? getAmazonLink(targetGpuName) : getHeurekaLink(targetGpuName, 'graficke-karty')} target="_blank" rel="nofollow sponsored noopener noreferrer" style={{ background: isEn ? '#f59e0b' : '#3b82f6', color: isEn ? '#000' : '#fff', padding: '16px', borderRadius: '12px', textDecoration: 'none', fontWeight: 950, width: '100%', textAlign: 'center' }}>
            ...
            <div className="affiliate-col" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
                <div style={{ color: '#a855f7', fontWeight: 950, fontSize: '13px', textTransform: 'uppercase' }}>
                    <Zap size={16} style={{ verticalAlign: 'middle', marginRight: '8px' }} />
                    {isEn ? 'CPU UPGRADE' : 'DOPORUČENÝ UPGRADE PROCESORU'}
                </div>
                <div style={{ opacity: 0.6, fontSize: '12px' }}>{isEn ? 'From $299' : 'Guru cena od 6 490 Kč'}</div>
                <div style={{ fontSize: '11px', color: '#ef4444', fontWeight: 900 }}>
                    {isEn ? 'CPU limits your GPU potential' : 'Procesor brzdí grafiku'}
                </div>
                <div style={{ background: 'rgba(34,197,94,0.1)', borderRadius: '12px', padding: '12px', fontWeight: 900, width: '100%', textAlign: 'center', border: '1px solid rgba(34,197,94,0.2)', color: '#22c55e' }}>
                    🚀 {isEn ? '+35% smoother gaming' : '+35% plynulejší hraní'}
                </div>
                <div style={{ fontWeight: 900, color: '#a855f7' }}>🔥 {targetCpuName}</div>
                <a href={isEn ? getAmazonLink(targetCpuName) : getHeurekaLink(targetCpuName, 'procesory')} target="_blank" rel="nofollow sponsored noopener noreferrer" style={{ background: isEn ? '#f59e0b' : '#3b82f6', color: isEn ? '#000' : '#fff', padding: '16px', borderRadius: '12px', textDecoration: 'none', fontWeight: 950, width: '100%', textAlign: 'center' }}>
                    {isEn ? 'AMAZON DEALS' : 'NAJÍT NEJLEVNĚJŠÍ CENU'}
                </a>
            </div>
        </section>
        ...
        <div style={{ marginTop: '40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            ...
            <a href={isEn ? "/en/bottleneck-calculator" : "/bottleneck-kalkulacka"} style={{ background: '#0a0b0d', border: '1px solid #a855f7', padding: '20px', borderRadius: '15px', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '15px' }}>
                ...
                <span style={{ fontSize: '16px', fontWeight: '950', color: '#fff' }}>{isEn ? 'BOTTLENECK TEST' : 'BOTTLENECK TEST'}</span>
            </a>
        </div>
        ...
        <div style={{ marginTop: '60px' }}>
            <BottleneckFatContent 
                cpuName={cpu.name} 
                gpuName={gpu.name} 
                gameName={gameParts[1]?.replace(/-/g, ' ').toUpperCase() || (isEn ? 'MODERN TITLES' : 'MODERNÍCH HRÁCH')} 
