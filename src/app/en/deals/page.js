...
import GameSearchWidget from '../../../components/GameSearchWidget';
...
export async function generateMetadata() {
  return {
    title: 'Best Game Deals & Discounts | The Hardware Guru',
    alternates: {
      canonical: 'https://thehardwareguru.cz/en/deals',
      ...
      <main className="inner-container" style={{ maxWidth: '1200px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
        ...
        <header style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 className="main-title" style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', fontWeight: '950', textTransform: 'uppercase', margin: '0', lineHeight: '1.1' }}>
            GAME <span style={{ color: '#f97316', textShadow: '0 0 30px rgba(249, 115, 22, 0.5)' }}>DEALS</span>
          </h1>
          <p className="desc-p" style={{ marginTop: '20px', color: '#9ca3af', fontSize: '18px', maxWidth: '700px', margin: '20px auto 0' }}>
            Hand-picked game keys and hardware discounts.
          </p>
        </header>
        ...
        {safeDeals.length === 0 ? (
            /* 🔥 NOVÝ HERNÍ VYHLEDÁVAČ MÍSTO PRÁZDNÉHO BOXU 🔥 */
            ...
            <div className="deals-grid">
                {safeDeals.map((deal, index) => {
                    const title = deal.title_en || deal.title;
                    const desc = deal.description_en || deal.description_cs;
                    const price = deal.price_en || deal.price_cs;
                        ...
                        <React.Fragment key={deal.id}>
                            <a href={deal.affiliate_link || '#'} target="_blank" rel="nofollow sponsored" className="deal-card">
                                <div className="deal-image-wrapper">
                                    ...
                                    <div className="deal-price-badge">{price || 'Price'}</div>
                                </div>
                                <div className="deal-content">
                                    <div className="deal-date"><Calendar size={12} /> {new Date(deal.created_at).toLocaleDateString('en-US')}</div>
                                    <h3 className="deal-title">{title}</h3>
                                    <p className="deal-desc">{desc}</p>
                                    <div className="deal-cta">
                                        <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><ShoppingCart size={16} /> GET DEAL</span>
        ...
        <div className="footer-support-section" style={{ marginTop: '80px', paddingTop: '50px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '25px' }}>
          <h4 style={{ color: '#9ca3af', fontSize: '15px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px', margin: 0, textAlign: 'center' }}>
            Support Guru by checking these offers.
          </h4>
          <div className="footer-btns" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '20px', width: '100%' }}>
            <a href="/en/support" className="guru-support-btn">
              <Heart size={20} /> SUPPORT GURU
            </a>
          </div>
        </div>
