...
import { headers } from 'next/headers'; // 🔥 PŘIDÁNO: Detekce jazyka
import Link from 'next/link';
import Script from 'next/script';
import { 
 Gamepad2, Monitor, ChevronLeft, ChevronRight, Zap, Swords, 
 ShoppingCart, Activity, CheckCircle2, ArrowRight, Flame, Heart, 
 BarChart3, Gauge, Trophy, Info, Crosshair, AlertTriangle
} from 'lucide-react';
...
export const dynamic = "force-dynamic"; // 🔥 FIX PRO CACHED 404
...
const findGpuBySlug = async (gpuSlug) => {
  ...
  try {
      // 🔥 cache: 'no-store' aby si to nepamatovalo 404
...
export async function generateMetadata(props) {
  ...
  const cleanSlug = rawSlug.replace(/^(en-|en\/)/, '');
  ...
  const canonicalUrl = isEn ? `${baseUrl}/en/gpu-fps/${safeSlug}` : `${baseUrl}/gpu-fps/${safeSlug}`;
  
  return {
    title: isEn ? `How much FPS does ${gpu.name} get? | Guru Benchmarks` : `Kolik FPS má ${gpu.name} ve hrách? | Guru Testy`,
    alternates: { canonical: canonicalUrl, languages: { 'en': `${baseUrl}/en/gpu-fps/${safeSlug}`, 'cs': `${baseUrl}/gpu-fps/${safeSlug}` } }
  };
}
...
export default async function GpuFpsHunterPage(props) {
  ...
  const gpu = await findGpuBySlug(cleanSlug);
  if (!gpu) notFound(); // Nyní už najde i anglické slugy správně
