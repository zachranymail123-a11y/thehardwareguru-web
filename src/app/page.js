...
// Tato funkce běží přímo na serveru a nevolá žádné vnitřní API = ŽÁDNÝ CRASH
async function getLiveStatus() {
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      // Voláme OpenAI přímo v této funkci (ne přes fetch /api/...), aby Netlify build nepadal
      if (openaiKey) {
        try {
          ...
          const completion = await openai.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
              { role: "system", content: "Jsi The Hardware Guru, 45letý HW nadšenec. Krátce a nadšeně (max 2 věty) popiš hru podle názvu. Buď gamer, používej slang." },
...
export default async function Home() {
  ...
  // 1. PŘIČTEME NÁVŠTĚVU (Bezpečně s catch)
  supabase.rpc('increment_total_visits').catch(() => {});

  // 2. STÁHNEME VŠE NAJEDNOU (Live Status + Články + Počítadlo)
  ...
  return (
    <div style={{ 
      ...
      <footer style={{ background: 'rgba(31, 40, 51, 0.95)', padding: '40px 20px', textAlign: 'center', borderTop: '2px solid #66fcf1', marginTop: '60px' }}>
          ...
          <p style={{ color: '#45a29e', opacity: 0.7, fontSize: '0.8rem', marginTop: '10px' }}>© 2026 The Hardware Guru.</p>
      </footer>
    </div>
  );
}
