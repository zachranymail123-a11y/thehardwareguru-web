...
// Funkce pro zjištění, zda streamuješ (YouTube API)
async function getLiveStatus() {
  const apiKey = process.env.YOUTUBE_API_KEY;
  const channelId = "UCgDdszBhhpqkNQc6t4YOCNw";
  
  // OPRAVA: Určení baseUrl pro volání vnitřního API
  let baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.URL || "";
  baseUrl = baseUrl.replace(/\/$/, ""); // Odstraníme lomítko na konci
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      // POJISTKA: Voláme AI API jen pokud máme baseUrl, aby web nespadl s "server-side exception"
      if (baseUrl) {
          try {
              ...
              console.error("AI Info skip - web jede dál v nouzovém režimu");
          }
      }
...
export default async function Home() {
  ...
  // Načítání statusu a dat paralelně bez zbytečných awaitů navíc
    ...
    <div style={{ 
      ...
      {liveStatus.isLive && (
        <section style={{ maxWidth: '1200px', margin: '40px auto', padding: '30px', background: 'rgba(255, 0, 0, 0.05)', borderRadius: '15px', border: '2px solid #ff0000', boxShadow: '0 0 30px rgba(255, 0, 0, 0.2)', textAlign: 'center' }}>
          ...
          <p style={{ color: '#e0e0e0', fontSize: '1.1rem', lineHeight: '1.6', fontStyle: 'italic', borderLeft: '4px solid #ff0000', paddingLeft: '20px', maxWidth: '800px', margin: '0 auto 20px auto' }}>
            {liveStatus.gameDesc}
          </p>
      ...
      <header style={{ maxWidth: '1200px', margin: '60px auto', padding: '40px', background: 'linear-gradient(145deg, rgba(31, 40, 51, 0.95), rgba(11, 12, 16, 0.95))', borderRadius: '15px', border: '1px solid #45a29e', display: 'flex', alignItems: 'center', gap: '40px', flexWrap: 'wrap', boxShadow: '0 0 20px rgba(0,0,0,0.5)' }}>
        <div style={{ flex: '1', minWidth: '300px' }}>
            ...
            <p style={{ fontSize: '1.1rem', lineHeight: '1.8', marginBottom: '30px', color: '#e0e0e0' }}>Čau pařani! Jsem 45letý HW nadšenec, gamer a streamer. Tady najdeš vše o hardwaru a záznamy z mých streamů.</p>
