 ...
 * Cesta: src/app/[locale]/page.js
 * 🛡️ FIX: Úplně odstraněny importy `next/link`, `next/navigation` a `@supabase/supabase-js`.
 * Kód nyní používá nativní `fetch` a standardní HTML tagy `<a>`, což garantuje 100% stabilitu v náhledu i na Vercelu.
 */
    ...
    async function fetchData() {
      try {
        ...
        const [p, s, t, tw, d, pa, exp, feat, duelsRes, cpuDuelsRes] = await Promise.all([
          ...
          fetch(`${supabaseUrl}/rest/v1/tweaky?select=*&order=created_at.desc&limit=3`, { headers: getHeaders }).then(res => res.json()),
          fetch(`${supabaseUrl}/rest/v1/darci?select=*&order=amount.desc&limit=20`, { headers: getHeaders }).then(res => res.json()),
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                    {data.featuredDeals.map(deal => (
                        <a key={deal.id} href={deal.affiliate_link} target="_blank" rel="nofollow sponsored" className="deal-hp-card group">
                            <img src={deal.image_url} style={{ width: '100px', height: '60px', borderRadius: '12px', objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)' }} alt={deal.title} />
                            <div style={{ flex: 1 }}>
