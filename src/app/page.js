...
// Stabilní detekce Live statusu bez shazování webu
async function getLiveStatus() {
  const apiKey = process.env.YOUTUBE_API_KEY;
  const channelId = "UCgDdszBhhpqkNQc6t4YOCNw";
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || (process.env.URL ? process.env.URL : "");
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      // POKUD JE ADRESA WEBU, ZKUSÍME AI, ALE KDYŽ SELŽE (Application error), WEB POJEDE DÁL
      if (baseUrl) {
          try {
              const cleanUrl = baseUrl.replace(/\/$/, "");
              const aiRes = await fetch(`${cleanUrl}/api/game-info?game=${encodeURIComponent(gameGuess)}`, { next: { revalidate: 3600 } });
              if (aiRes.ok) {
                  const aiData = await aiRes.json();
                  aiDescription = aiData.description;
              }
          } catch (e) {
              console.log("AI Info skip - prevence Application erroru");
          }
      }
...
export default async function Home() {
  ...
  // Spuštění všeho paralelně - přesně jako ve tvojí funkční verzi
    ...
    <div style={{ minHeight: '100vh', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", color: '#c5c6c7', backgroundImage: "linear-gradient(rgba(11, 12, 16, 0.92), rgba(11, 12, 16, 0.85)), url('https://i.postimg.cc/QdWxszv3/bg-guru.png')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      ...
      <header style={{ maxWidth: '1200px', margin: '60px auto', padding: '40px', background: 'linear-gradient(145deg, rgba(31, 40, 51, 0.95), rgba(11, 12, 16, 0.95))', borderRadius: '15px', border: '1px solid #45a29e', display: 'flex', alignItems: 'center', gap: '40px', flexWrap: 'wrap', boxShadow: '0 0 20px rgba(0,0,0,0.5)' }}>
        <div style={{ flex: '1', minWidth: '300px' }}>
            ...
            <p style={{ fontSize: '1.1rem', lineHeight: '1.8', marginBottom: '30px', color: '#e0e0e0' }}>Čau pařani! Jsem 45letý HW nadšenec, gamer a streamer. Tady najdeš vše o hardwaru a záznamy záznamy ze streamů. Na Kicku mi sekunduje unikátní AI umělá inteligence.</p>
