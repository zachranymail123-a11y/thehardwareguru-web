...
// Tato funkce běží přímo na serveru, nevolá žádné vnitřní API = ŽÁDNÝ CRASH
async function getLiveStatus() {
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      // Přímé volání OpenAI bez vnitřního API fetche
      if (openaiKey) {
        try {
          ...
          const completion = await openai.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
              { role: "system", content: "Jsi The Hardware Guru, 45letý HW nadšenec. Krátce (max 2 věty) popiš hru a buď nadšený gamer." },
    ...
    <div style={{ 
      ...
      {liveStatus.isLive && (
        <section style={{ maxWidth: '1200px', margin: '40px auto', padding: '30px', background: 'rgba(255, 0, 0, 0.05)', borderRadius: '15px', border: '2px solid #ff0000', boxShadow: '0 0 30px rgba(255, 0, 0, 0.2)', textAlign: 'center' }}>
          ...
          <p style={{ color: '#e0e0e0', fontSize: '1.1rem', lineHeight: '1.6', fontStyle: 'italic', maxWidth: '800px', margin: '0 auto 20px' }}>
            {liveStatus.gameDesc}
          </p>
