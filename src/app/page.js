...
/**
 * GURU HOMEPAGE V19.5 - THE FINAL MONEY UPDATE
 * 🚀 CÍL: 100% integrita zálohy V18.6 + Seznam Ad + Dlouhé Partner CTA pod Hero.
 */
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', color: '#fff', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', fontFamily: 'sans-serif' }}>
      ...
      <style>{`
        ...
        .guru-partner-button {
          background: linear-gradient(90deg, rgba(168, 85, 247, 0.2) 0%, rgba(102, 252, 241, 0.2) 100%);
        ...
        .guru-partner-button:hover {
          transform: scale(1.02);
          border-color: #a855f7;
          background: rgba(168, 85, 247, 0.3);
          box-shadow: 0 20px 50px rgba(168, 85, 247, 0.3);
        }
        .partner-btn-text { font-size: clamp(14px, 2vw, 18px); font-weight: 950; color: #fff; text-transform: uppercase; letter-spacing: 1px; display: flex; align-items: center; gap: 15px; }

        @media (max-width: 768px) { .ad-desktop-wrapper { display: none; } .ad-mobile-wrapper { display: flex; justify-content: center; } }
      `}</style>

      {/* --- 🚀 TOP REKLAMA (Money-Fix) --- */}
      ...
      {/* --- 🚀 HERO SEKCE (RANNÍ V18.6 - ORIGINÁL) --- */}
      <header className="guru-hero-section">
        <div style={{ flex: '1', minWidth: '300px', position: 'relative', zIndex: 10 }}>
            ...
            <p style={{ fontSize: '1.15rem', lineHeight: '1.8', color: '#9ca3af', marginBottom: '40px', maxWidth: '700px' }}>
              {isEn ? "Hardware expert with 20 years of experience. Mission: eradicate lag, optimize FPS, and build a place where every geek feels at home." : "S 20 lety praxe v servisu hardware vím, kde každá mašina tlačí. Moje mise je jasná: vymýtit lagy, zkrotit FPS a vytvořit web, kde se každý geek cítí jako doma."}
            </p>
      ...
      {/* --- 🚀 DLOUHÉ PARTNER TLAČÍTKO (Pod panelem) --- */}
      <section style={{ maxWidth: '1200px', margin: '20px auto 60px auto', padding: '0 20px' }}>
      ...
      {/* --- HARD TEXT BLOCK (RANNÍ V18.6 - ORIGINÁL) --- */}
      <section className="seo-hard-text-block" style={{ marginBottom: '60px' }}>
        {isEn ? (
          <>
            <h2>Hardware Guru – CPU, GPU comparison and FPS calculator</h2>
            <p>Welcome to The Hardware Guru, a leading tech portal focused on detailed <a href="/en/gpuvs">graphics card (GPU) comparisons</a> and <a href="/en/cpuvs">processor (CPU) comparisons</a>.</p>
      ...
      {/* --- SEO: CATEGORY HUB (RANNÍ V18.6 - ORIGINÁL) --- */}
      <section style={{ maxWidth: '1200px', margin: '0 auto 60px auto', padding: '0 20px', textAlign: 'center' }}>
          <a href="/clanky" className="seo-link-pill"><Compass size={14} style={{ marginRight: '5px', display: 'inline', verticalAlign: 'middle' }}/> HW Novinky</a>
          <a href="/gpu-index" className="seo-link-pill"><Layers size={14} style={{ marginRight: '5px', display: 'inline', verticalAlign: 'middle' }}/> GPU Katalog</a>
          <a href="/cpu-index" className="seo-link-pill"><Cpu size={14} style={{ marginRight: '5px', display: 'inline', verticalAlign: 'middle' }}/> CPU Katalog</a>
          <a href="/slovnik" className="seo-link-pill"><MessageSquare size={14} style={{ marginRight: '5px', display: 'inline', verticalAlign: 'middle' }}/> HW Slovník</a>
          <a href="/sestavy" className="seo-link-pill"><Gamepad2 size={14} style={{ marginRight: '5px', display: 'inline', verticalAlign: 'middle' }}/> PC Sestavy</a>
      </section>

      {/* --- SDÍLET & ODEBÍRAT (RANNÍ V18.6 - ORIGINÁL) --- */}
      <section style={{ maxWidth: '1200px', margin: '0 auto 60px auto', background: 'rgba(15, 17, 21, 0.95)', border: '1px solid rgba(168, 85, 247, 0.3)', borderRadius: '24px', padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px', backdropFilter: 'blur(10px)', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <button id="guru-os-btn" className="hover-scale" style={{ width: '48px', height: '48px', borderRadius: '16px', border: 'none', background: '#2563eb', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Bell size={20} fill="#fff" /></button>
      ...
      {/* --- MONETIZACE A VIZUÁLNÍ SEKCE (RANNÍ V18.6 - ORIGINÁL) --- */}
        ...
        {data.darci.length > 0 && data.partneri.length > 0 && (
          <section style={{ display: 'flex', gap: '30px', flexWrap: 'wrap', marginBottom: '60px' }}>
              <a href={isEn ? "/en/sin-slavy" : "/sin-slavy"} className="monetize-hero-card" style={{ flex: 1, minWidth: '300px' }}>
                ...
                <p style={{ color: '#9ca3af', fontSize: '15px', lineHeight: '1.5', fontWeight: 'bold' }}>{data.darci.slice(0, 5).map(d => d.name).join(', ')}...</p>
              </a>
          </section>
        )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
              {data.featuredDeals.map(deal => (
                <a key={deal.id} href={deal.affiliate_link} target="_blank" rel="nofollow sponsored" className="deal-hp-card">
                  <img src={deal.image_url} alt="" style={{ width: '100px', height: '60px', borderRadius: '12px', objectFit: 'cover' }} />
