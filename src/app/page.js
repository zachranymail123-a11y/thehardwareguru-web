...
// Logika pro detekci streamu a AI popisu - běží přímo na serveru
async function getLiveStatus() {
  ...
  try {
    ...
    if (ytData.items && ytData.items.length > 0) {
      ...
      // 2. Přímé OpenAI volání (pokud máme klíč)
      if (openaiKey) {
        try {
          ...
          const completion = await openai.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
              { role: "system", content: "Jsi The Hardware Guru, 45letý HW nadšenec. Krátce a úderně (max 2 věty) popiš hru. Buď nadšený gamer, používej slang." },
...
export default async function Home() {
  ...
  // 1. PŘIČTEME NÁVŠTĚVU (s pojistkou)
  supabase.rpc('increment_total_visits').catch(() => {});

  // 2. STÁHNEME DATA + LIVE STATUS NAJEDNOU
