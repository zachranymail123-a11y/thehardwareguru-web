...
// TATO FUNKCE TEĎ DĚLÁ VŠE NAJEDNOU A PŘÍMO NA SERVERU
async function getLiveStatus() {
  ...
  try {
    // 1. KONTROLA YOUTUBE
    const ytUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&type=video&eventType=live&key=${ytKey}`;
    const ytRes = await fetch(ytUrl, { next: { revalidate: 300 } });
    const ytData = await ytRes.json();
    
    if (ytData.items && ytData.items.length > 0) {
      const streamTitle = ytData.items[0].snippet.title;
      const gameGuess = streamTitle.split('|')[0].trim();
      let aiDescription = "Právě streamujeme tuhle pecku! Doraz pokecat do chatu a mrknout na gameplay.";

      // 2. SERPER (Fakta o hře) - Jen pokud máme klíč
      ...
      if (serperKey) {
        try {
          const serperRes = await fetch('https://google.serper.dev/search', {
            ...
            body: JSON.stringify({ q: `${gameGuess} video game release date story genre` })
          });
          const searchData = await serperRes.json();
          context = searchData.organic?.[0]?.snippet || context;
        } catch (e) { console.error("Serper skip"); }
      }

      // 3. OPENAI (Guru popis) - Jen pokud máme klíč
      if (openaiKey) {
        try {
          ...
          const completion = await openai.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
              { role: "system", content: "Jsi The Hardware Guru, 45letý HW nadšenec. Krátce a úderně (2 věty) popiš hru. Buď gamer, slang." },
              { role: "user", content: `Hra: ${gameGuess}. Kontext: ${context}` }
            ],
            max_tokens: 100
          });
          aiDescription = completion.choices[0].message.content;
        } catch (e) { console.error("OpenAI skip"); }
      }
...
export default async function Home() {
  ...
  // Přesně tvůj původní funkční kód pro načítání dat
  ...
  return (
    <div style={{ 
      ...
      <header style={{ maxWidth: '1200px', margin: '60px auto', padding: '40px', background: 'linear-gradient(145deg, rgba(31, 40, 51, 0.95), rgba(11, 12, 16, 0.95))', borderRadius: '15px', border: '1px solid #45a29e', display: 'flex', alignItems: 'center', gap: '40px', flexWrap: 'wrap', boxShadow: '0 0 20px rgba(0,0,0,0.5)' }}>
        <div style={{ flex: '1', minWidth: '300px' }}>
            ...
            <div style={{ display: 'flex', gap: '15px' }}>
                <a href="https://kick.com/thehardwareguru" target="_blank" className="social-btn">STREAM NA KICKU</a>
            </div>
        </div>
      ...
      <footer style={{ background: 'rgba(31, 40, 51, 0.95)', padding: '40px 20px', textAlign: 'center', borderTop: '2px solid #66fcf1', marginTop: '60px' }}>
          <div style={{ color: '#66fcf1', fontWeight: 'bold' }}>WEB NAVŠTÍVILO JIŽ {celkemNavstev} FANOUŠKŮ 🦾</div>
      </footer>
    </div>
  );
}
