...
/**
 * GURU GLOSSARY ENGINE - DETAIL V2.4 (MOBILE OPTIMIZED & ADS SEPARATION)
 * 🚀 CÍL: 100% monetizace slovníkových pojmů a perfektní mobilní UX pro technický výklad.
 */
...
export default async function SlovnikDetail({ params }) {
  ...
  // Google Golden Rich: Combined Article & Product Schema
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": title,
    "description": seoDesc,
    "image": term.image_url ? [term.image_url] : [`${baseUrl}/logo.png`],
    "datePublished": term.created_at,
    "author": { "@type": "Person", "name": "The Hardware Guru", "url": baseUrl }
  };

  const safeJson = (obj) => JSON.stringify(obj).replace(/</g, '\\u003c');

  return (
    <div className="guru-term-wrapper" style={{ 
      ...
      <main className="inner-container" style={{ 
        ...
        {term.image_url && (
          <div className="term-hero" style={{ width: '100%', height: '450px', position: 'relative', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="term-content-wrapper" style={{ padding: '40px 50px 60px 50px' }}>
          ...
          <header style={{ marginBottom: '50px', textAlign: 'center' }}>
            <div className="meta-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px', color: '#9ca3af', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '25px' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#66fcf1' }}><ShieldCheck size={16} /> GURU WIKI</span>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .share-btn-x { display: flex; align-items: center; gap: 8px; background: #1da1f220; color: #1da1f2; border: 1px solid #1da1f250; padding: 10px 20px; border-radius: 12px; font-weight: 950; text-decoration: none; transition: 0.3s; }
        .share-btn-fb { display: flex; align-items: center; gap: 8px; background: #1877f220; color: #1877f2; border: 1px solid #1877f250; padding: 10px 20px; border-radius: 12px; font-weight: 950; text-decoration: none; transition: 0.3s; }
        ...
        .guru-prose h2 { color: #fff; font-size: 2rem; font-weight: 950; margin-top: 2.5em; text-transform: uppercase; border-left: 5px solid #66fcf1; padding-left: 20px; }
        ...
        @media (max-width: 768px) {
          .guru-term-wrapper { padding-top: 80px !important; }
          .inner-container { border-radius: 0 !important; border: none !important; }
          .term-hero { height: 250px !important; }
          .term-content-wrapper { padding: 30px 20px 60px 20px !important; }
          ...
          .share-btn-x, .share-btn-fb { width: 100%; justify-content: center; }
