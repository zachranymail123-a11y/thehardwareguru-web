    ...
    <div style={globalStyles}>
      <header style={{ maxWidth: '800px', margin: '20px auto 40px', textAlign: 'center', padding: '0 20px' }}>
        ...
        <p style={{ marginTop: '15px', color: '#9ca3af', fontWeight: '600', fontSize: '18px' }}>
          {isEn ? 'Technical terms decoded for every geek.' : 'Technické pojmy a zkratky dešifrované do lidské řeči.'}
        </p>
      </header>
            ...
            {items.map((item) => {
              ...
              return (
                <Link key={item.id} href={isEn ? `/en/slovnik/${displaySlug}` : `/slovnik/${displaySlug}`} prefetch={false} style={{ textDecoration: 'none' }}>
                  <div style={{ background: 'rgba(17, 19, 24, 0.85)', padding: '30px', borderRadius: '24px', border: '1px solid rgba(168, 85, 247, 0.2)', height: '100%', display: 'flex', flexDirection: 'column', transition: '0.3s' }} onMouseEnter={e => e.currentTarget.style.borderColor = '#a855f7'} onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(168, 85, 247, 0.2)'}>
                    <div style={{ color: '#a855f7', marginBottom: '15px' }}><BookOpen size={32} /></div>
                    ...
                    <div style={{ color: '#a855f7', fontWeight: '900', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                      {isEn ? 'READ DEFINITION' : 'ČÍST DEFINICI'} <ChevronRight size={16} />
                    </div>
                  </div>
                </Link>
              );
