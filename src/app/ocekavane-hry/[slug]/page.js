...
import { Monitor, ArrowLeft, Loader2, Zap, Activity, Share2, Play, Info } from 'lucide-react';
import Link from 'next/link';

/**
 * 🚀 GURU EXPECTED GAME DETAIL ENGINE - MASTER V3
 * Srdce sekce Očekávané hry. 
 * Obsahuje: Dual-lang slug engine, Auto-video detection a Elite Neon UI.
 */
  ...
  return (
    <div style={pageWrapper}>
      <style>{`
        ...
        .guru-master-video { margin: 50px 0; border: 2px solid #66fcf1; border-radius: 32px; overflow: hidden; background: #000; box-shadow: 0 0 50px rgba(102, 252, 241, 0.2); }
      `}</style>
      ...
      <article style={container}>
        <div style={contentBoxStyle}>
          ...
          {/* 🚀 GURU MASTER VIDEO (Zobrazíme nahoře, pokud existuje trailer) */}
          {item.trailer && (
             <div className="guru-master-video">
                <div style={{ padding: '20px 30px', background: 'rgba(102, 252, 241, 0.1)', display: 'flex', alignItems: 'center', gap: '12px', color: '#66fcf1', fontWeight: '900' }}>
                   <Play size={20} fill="#66fcf1" /> {isEn ? 'TECHNICAL VIDEO FOOTAGE' : 'TECHNICKÉ VIDEO / TRAILER'}
                </div>
                <div style={{ aspectRatio: '16/9' }}>
                   {item.trailer.includes('youtube.com') || item.trailer.includes('embed')
                     ? <iframe width="100%" height="100%" src={item.trailer} frameBorder="0" allowFullScreen></iframe>
                     : <video width="100%" height="100%" controls poster={item.image_url} style={{ display: 'block' }}><source src={item.trailer} type="video/mp4" /></video>
                   }
                </div>
             </div>
          )}
          ...
          {/* 🖼️ HERO IMAGE (Zobrazíme, jen pokud nemáme video, aby to nebylo duplicitní) */}
