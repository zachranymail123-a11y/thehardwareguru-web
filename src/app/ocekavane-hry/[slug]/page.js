...
export default async function ExpectedGameDetailPage(props) {
    const params = await props.params;
    const rawSlug = params?.slug || '';
    const isEn = rawSlug.startsWith('en-');
    const post = await getPost(rawSlug);
        ...
        <div className="guru-article-wrapper" style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
            ...
            <main className="inner-container" style={{ maxWidth: '900px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
                ...
                <article className="main-article-card" style={{ background: 'rgba(15, 17, 21, 0.95)', padding: '50px 40px', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.05)', boxShadow: '0 20px 50px rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)' }}>
                    <header style={{ marginBottom: '30px', textAlign: 'center' }}>
                        ...
                        <h1 className="article-h1" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: '950', lineHeight: '1.2', margin: '0 0 20px 0', textTransform: 'uppercase' }}>
                            {title}
                        </h1>
                ...
                <div style={{ marginTop: '50px' }}>
                    <div className="share-grid">
                        ...
                        <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="share-card fb-bg">
                            <Share2 size={18} /> FACEBOOK
                        </a>
                        <a href={`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(title)}`} target="_blank" rel="noopener noreferrer" className="share-card reddit-bg">
                ...
                {latestPosts.length > 0 && (
                    <section style={{ marginTop: '60px' }}>
                        ...
                        <div className="related-grid">
                            {latestPosts.map((lp) => (
                                <a key={lp.slug} href={isEn ? `/en/ocekavane-hry/${lp.slug_en || lp.slug}` : `/ocekavane-hry/${lp.slug}`} className="related-card">
            ...
            <style dangerouslySetInnerHTML={{__html: `
                ...
                @media (max-width: 768px) {
                    .guru-article-wrapper { padding-top: 80px !important; }
                    .inner-container { padding: 0 15px !important; }
                    .main-article-card { padding: 30px 20px !important; border-radius: 20px !important; }
                    .article-h1 { font-size: 1.8rem !important; }
                    .guru-article-content { font-size: 1rem !important; }
                    .gta6-conversion-box { padding: 25px 15px !important; }
