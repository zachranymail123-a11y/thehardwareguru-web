...
/**
 * 🚀 GURU EXPECTED GAMES ARCHIVE
 * Přehled budoucích technických milníků. 
 * GURU UPDATE: Přidány popisky her přímo do karet pro lepší UX.
 */
    ...
    async function fetchData() {
      try {
        const { data, error } = await supabase
          .from('posts')
          .select('*')
          .eq('type', 'expected') // GURU CORE: Filtrujeme pouze technické preview budoucích her
                ...
                <Link key={item.id} href={isEn ? `/en/ocekavane-hry/${displaySlug}` : `/ocekavane-hry/${displaySlug}`} style={{ textDecoration: 'none' }}>
                  <article className="expected-card">
                    <div className="card-image-wrapper">
                       <img src={item.image_url} alt={displayTitle} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.8 }} />
                       <div style={techBadge}>
                    ...
                    <div style={{ padding: '30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                      ...
                      {/* 📝 GURU BRIEF: Popisek o čem hra bude */}
                      <p className="desc-text">
                        {displayDesc || (isEn ? 'Technical analysis of engine and performance expectations.' : 'Technická analýza enginu a očekávaného výkonu na PC.')}
                      </p>
