...
/**
 * GURU GPU DUELS ENGINE - DETAIL V80.0 (SUPREME SEO & STABILITY)
 * Cesta: src/app/gpuvs/[slug]/page.js
 * 🛡️ FIX 1: Bezpečný fetch similar duelů (ochrana proti gpuA=null).
 * 🛡️ FIX 2: Agresivní slugify odstraňuje vendor/graphics/gpu klíčová slova.
 * 🛡️ SEO FIX 1: ItemList schema nyní obsahuje povinný parametr "name".
 * 🛡️ SEO FIX 2: "Quick Verdict Badge" pod H1 pro získání Featured Snippets.
 * 🚀 PERF NOTE: Pro produkci se doporučuje v DB vytvořit materialized view 'gpu_full'.
 */
...
// 🛡️ GURU ENGINE: Načtení podobných duelů (Matrix Linking)
...
// 🚀 GURU ENGINE: Generování Programmatic SEO obsahu (pokud neexistuje)
...
export async function generateMetadata({ params }) {
  ...
  if (winner) {
      ...
      desc = isEn 
        ? `${winner.name} is about ${diff}% faster than ${loser.name} based on benchmark data. Compare specs, gaming FPS and real performance.` 
...
export default async function GpuDuelDetail({ params }) {
  ...
  // 🚀 GURU FIX: Bezpečný fetch similar duelů
    ...
    "mainEntity": [
      {
        ...
        "acceptedAnswer": { 
            "@type": "Answer", 
            "text": winner 
              ? (isEn ? `${winner.name} is about ${finalPerfDiff}% faster in gaming benchmarks.` : `Ano, ${winner.name} je v herních benchmarcích přibližně o ${finalPerfDiff} % výkonnější.`)
              : (isEn ? "Both GPUs offer very similar gaming performance." : "Obě grafické karty nabízejí velmi vyrovnaný herní výkon.")
        }
      },
      {
        "@type": "Question",
        "name": isEn ? `Is ${gpuA.name} worth upgrading from ${gpuB.name}?` : `Vyplatí se upgrade z ${gpuB.name} na ${gpuA.name}?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": isEn 
            ? `${gpuA.name} offers about ${finalPerfDiff}% higher gaming performance than ${gpuB.name}.` 
            : `${gpuA.name} nabízí přibližně o ${finalPerfDiff} % vyšší herní výkon než ${gpuB.name}.`
        }
      }
    ]
  };
  ...
  // 🚀 GURU: ItemList schema pro podobné linky (ChatGPT Fix - přidáno "name")
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0b0d', backgroundImage: 'url("/bg-guru.png")', backgroundSize: 'cover', backgroundAttachment: 'fixed', paddingTop: '120px', paddingBottom: '100px', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', width: '100%', padding: '0 20px' }}>
        ...
        <header style={{ marginBottom: '50px', textAlign: 'center' }}>
          ...
          <div style={{ color: '#9ca3af', fontSize: '18px', marginTop: '20px', maxWidth: '850px', margin: '20px auto', lineHeight: '1.6' }}>
            {winner ? (
                isEn 
                ? <p>{winner.name} is approximately <strong>{finalPerfDiff}% faster</strong> than {loser.name} in gaming benchmarks based on aggregated technical data.</p>
                : <p>{winner.name} je přibližně o <strong>{finalPerfDiff} % výkonnější</strong> než {loser.name} v herních testech na základě agregovaných technických dat.</p>
            ) : (
                isEn
          ...
          {/* 🚀 GURU: QUICK VERDICT BADGE (ChatGPT SEO Hack) */}
          {winner && (
        ...
        {/* 🚀 GURU: H2 GAMING PERFORMANCE (SEO HACK) */}
        <section style={{ marginBottom: '60px' }}>
        ...
        {/* 🚀 GURU: ROZŠÍŘENÁ TABULKA SPECIFIKACÍ */}
        <section style={{ marginBottom: '60px' }}>
        ...
        {/* 🚀 GURU: INTERNAL LINK MATRIX "Srovnejte s..." */}
        {similar.length > 0 && (
          <section style={{ marginBottom: '60px' }}>
            <h2 className="section-h2" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .guru-ranking-link:hover { color: #fff; transform: translateY(-2px); }
        ...
        .guru-upgrade-pill:hover { background: #a855f7; color: #fff; transform: scale(1.05); box-shadow: 0 0 30px rgba(168, 85, 247, 0.4); }
        ...
        .guru-prose-style h2 { color: #fff; font-size: 2rem; font-weight: 950; margin-top: 2.5em; margin-bottom: 0.8em; text-transform: uppercase; border-left: 4px solid #ff0055; padding-left: 15px; }
        ...
        .similar-link-card:hover { transform: translateY(-3px); border-color: #66fcf1; color: #fff; background: rgba(102, 252, 241, 0.05); }
        ...
        @media (max-width: 768px) { .guru-grid-ring { grid-template-columns: 1fr !important; } .vs-badge { margin: 10px auto; rotate: 0deg; } .deals-btn-style, .support-btn-style { width: 100%; } .vs-badge { margin: 0; } .vs-center-wrapper { margin: 10px 0; } .table-label { width: 100px; } }
      `}} />
    </div>
  );
}
