...
export default function SupportPage() {
  ...
  // 🚀 GURU ANTI-SPA SHIELD & SWG REINITIALIZATION 🚀
  // Zajišťuje, že Google Subscribe tlačítko se stoprocentně zinicializuje i při klientské navigaci
  useEffect(() => {
    // 1. Pokusíme se skriptu vnutit novou inicializaci po příchodu na stránku
    ...
    // 2. Bezpečnostní pojistka: Zkontrolujeme, zda se do tlačítka vygeneroval iframe.
    // Pokud ne, vynutíme reload, aby se načetl Google script z hlavičky natvrdo.
    const timer = setTimeout(() => {
      const btn = document.getElementById('support-page-swg-btn');
      if (btn && !btn.querySelector('iframe') && !sessionStorage.getItem('guru_swg_reloaded')) {
         sessionStorage.setItem('guru_swg_reloaded', 'true');
         window.location.reload();
      } else {
         sessionStorage.removeItem('guru_swg_reloaded');
      }
    }, 1200);
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={cardStyle}>
        ...
        {/* 🚀 1. MOŽNOST: GOOGLE SUBSCRIBE (OPRAVENÝ OVERLAY & INIT LOGIKA) 🚀 */}
        <div style={{ marginBottom: '12px' }}>
          <div className="guru-btn-hover" style={{ ...buttonStyle('google'), position: 'relative', overflow: 'hidden' }}>
             ...
             {/* Neviditelná vrstva pro originální tlačítko s vlastní pojistkou proti kliku na prázdno */}
