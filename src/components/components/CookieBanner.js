...
export default function CookieBanner() {
  ...
  useEffect(() => {
    // 1. Zjistíme jazyk podle URL (podpora pro CZ a EN variantu webu)
    if (typeof window !== 'undefined' && window.location.pathname.startsWith('/en')) {
      setIsEn(true);
    }
    
    // 2. Zkontrolujeme, jestli už uživatel cookies odkliknul
  ...
  const handleAcceptAll = () => {
    ...
    setIsVisible(false);
    // Zde se později dají inicializovat reklamní skripty (Seznam, AdSense)
  };
