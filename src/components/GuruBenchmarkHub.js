...
import { Cpu, Zap, Play, MonitorPlay, AlertTriangle, ShieldCheck, User, Activity, Crosshair, Swords, BookOpen, ArrowRight } from 'lucide-react';
...
export default function GuruBenchmarkHub({ t, locale = 'cs' }) {
    ...
    const crossLinks = [
        { href: `${prefix}/bottleneck-calculator`, label: t.linkBottleneck, icon: <Activity size={24} />, color: '#ff0055' },
        { href: `${prefix}/fps-kalkulacka`, label: t.linkFps, icon: <Crosshair size={24} />, color: '#10b981' },
        { href: `${prefix}/cpu-index`, label: t.linkCpuDuels, icon: <Swords size={24} />, color: '#f59e0b' },
        { href: `${prefix}/gpu-index`, label: t.linkGpuDuels, icon: <Zap size={24} />, color: '#3b82f6' },
        { href: `${prefix}/clanky`, label: t.linkArticles, icon: <BookOpen size={24} />, color: '#a855f7' }
    ];

    return (
        <div style={{ background: '#0a0b0d', color: '#fff', minHeight: '100vh', padding: '40px 20px', fontFamily: 'sans-serif' }}>
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '40px' }}>
                    ...
                    <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', fontWeight: '950', textTransform: 'uppercase', margin: '0 0 10px 0', lineHeight: '1' }}>
                        {t.title}
                    </h1>
                ...
                <div style={{ background: 'rgba(15, 17, 21, 0.95)', border: '1px solid rgba(168, 85, 247, 0.3)', borderRadius: '24px', padding: '40px', boxShadow: '0 20px 50px rgba(0,0,0,0.5), inset 0 0 20px rgba(168, 85, 247, 0.05)', marginBottom: '40px' }}>
                    
                    {testPhase === 'idle' ? (
                        <form onSubmit={startFullBenchmark} style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                            <div style={{ textAlign: 'center', padding: '15px', background: 'rgba(16, 185, 129, 0.05)', borderRadius: '12px', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                            ...
                            <div>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff', fontWeight: '900', marginBottom: '10px', textTransform: 'uppercase', fontSize: '16px', letterSpacing: '1px' }}>
                                    <User size={20} color="#a855f7" /> {t.nickLabel} *
                                </label>
                                <input type="text" required value={nickname} onChange={(e) => setNickname(e.target.value)} placeholder={t.nickPlaceholder} style={{ width: '100%', padding: '20px', borderRadius: '12px', border: '2px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.5)', color: '#fff', fontSize: '18px', fontWeight: 'bold', outline: 'none', transition: 'border 0.3s' }} onFocus={(e) => e.target.style.borderColor = '#a855f7'} onBlur={(e) => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
                            </div>

                            <div>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff', fontWeight: '900', marginBottom: '10px', textTransform: 'uppercase', fontSize: '16px', letterSpacing: '1px' }}>
                                    <Cpu size={20} color="#ff0055" /> {t.cpuLabel}
                                </label>
                                <input type="text" value={cpuInput} onChange={(e) => setCpuInput(e.target.value)} placeholder={t.cpuPlaceholder} style={{ width: '100%', padding: '20px', borderRadius: '12px', border: '2px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.5)', color: '#fff', fontSize: '18px', outline: 'none', transition: 'border 0.3s' }} onFocus={(e) => e.target.style.borderColor = '#ff0055'} onBlur={(e) => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
                                <div style={{ fontSize: '14px', color: '#9ca3af', marginTop: '10px' }}><AlertTriangle size={14} style={{display:'inline', color: '#f59e0b'}}/> {t.cpuDesc}</div>
                            </div>
                            ...
                            <button type="submit" style={{ background: 'linear-gradient(90deg, #ff0055, #a855f7)', color: '#fff', padding: '25px', borderRadius: '16px', border: 'none', fontWeight: '950', fontSize: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px', marginTop: '10px', textTransform: 'uppercase', transition: '0.3s', boxShadow: '0 10px 40px rgba(168, 85, 247, 0.4)' }} className="hover:scale-105">
                                <Play size={28} fill="currentColor" /> {t.startBtn}
                            </button>
                        </form>
                    ) : (
                        <div style={{ padding: '60px 0', textAlign: 'center' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', marginTop: '40px' }}>
                                <div style={{ opacity: testPhase === 'cpu' ? 1 : 0.3, color: '#ff0055' }}><Cpu size={32} /></div>
                                <div style={{ opacity: testPhase === 'gpu' ? 1 : 0.3, color: '#10b981' }}><Zap size={32} /></div>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {/* MASIVNÍ PROLINKOVÁNÍ NA DALŠÍ NÁSTROJE */}
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '50px', paddingBottom: '50px' }}>
                    <h2 style={{ fontSize: '28px', fontWeight: '950', color: '#fff', marginBottom: '30px', textAlign: 'center', textTransform: 'uppercase' }}>
                        {t.exploreMore}
                    </h2>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
                        {crossLinks.map((link, idx) => (
                            <Link key={idx} href={link.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', color: '#fff', padding: '30px 20px', borderRadius: '16px', textDecoration: 'none', fontWeight: '900', fontSize: '16px', textAlign: 'center', transition: 'all 0.3s' }} className="hover:bg-white/10 hover:-translate-y-2" onMouseEnter={(e) => e.currentTarget.style.borderColor = link.color} onMouseLeave={(e) => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.05)'}>
