...
export default function Navbar() {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      // Hledáme shodu v titulku NEBO v popisu (najde i 'ddr' uvnitř textu)
      ...
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', margin: '0 20px', position: 'relative' }} ref={suggestionRef}>
        ...
        {showSuggestions && (suggestions.length > 0 || isLoading) && (
          <div style={{ 
            ...
            background: 'rgba(13, 14, 16, 0.95)', backdropFilter: 'blur(10px)',
            ...
            {isLoading ? (
              <div style={{ padding: '15px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>
                {isEn ? 'Searching the Guru database...' : 'Prohledávám Guru databázi...'}
              </div>
            ) : (
              suggestions.map((s, i) => (
                <div key={i} 
