...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        // GURU FIX: Přesná syntaxe pro Supabase .or() bez mezer, aby konzole neházela Error 400
          ...
          <div style={{ 
            ...
            {isLoading ? (
              <div style={{ padding: '15px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>
                {isEn ? 'Searching...' : 'Hledám v archivu...'}
              </div>
            ) : (
              suggestions.map((s, i) => (
                <div key={i} 
                  ...
                  style={{ 
                    padding: '12px 15px', borderBottom: i !== suggestions.length - 1 ? '1px solid #222' : 'none', 
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
          ...
          <Link href={isEn ? "/en/rady" : "/rady"} style={{ color: '#d1d5db', textDecoration: 'none', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '5px' }}>
            <HeartPulse size={14}/> {isEn ? 'GUIDES' : 'RADY'}
          </Link>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          {/* GURU FIX: INSTAGRAM NATVRDO MEZI YOUTUBE A DISCORD */}
