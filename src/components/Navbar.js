...
export default function Navbar() {
  ...
  // LOGIKA INTELIGENTNÍHO NAŠEPTÁVAČE S FIXEM CHYBY 400
  useEffect(() => {
    let active = true;
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        // GURU FIX: PostgREST (Supabase) vyžaduje u .or() řetězec ABSOLUTNĚ BEZ MEZER po čárkách.
        const searchTerm = `%${q}%`;
        ...
        if (active) {
          if (error) {
            console.error("Guru Search Error Fix:", error.message);
            // Fallback na čistý title search, pokud .or selže
  ...
  return (
    <nav style={{
      ...
      justifyContent: 'space-between', color: '#fff', height: '90px' // GURU FIX: Zvětšená výška na 90px
    }}>
      ...
      <Link href={isEn ? "/en" : "/"} style={{ textDecoration: 'none', flexShrink: 0 }}>
        <span style={{ 
          color: '#a855f7', 
          fontFamily: 'serif', 
          fontSize: '28px', // GURU FIX: Větší logo
          fontWeight: '900', 
          letterSpacing: '1px', 
      ...
      <div style={{ display: 'flex', gap: '30px', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
          ...
          {/* GURU FIX: Změněno z RADY na PRAKTICKÉ RADY */}
